...
import onera.pmlanalyzer.views.dependability.model.{Expr, TargetId, Variable, Worst}
...
trait RosaceDependabilitySpecification extends DependabilitySpecification {
  ...
  def softwareStoresDependency(p: Application):
  (Variable[OLE], Variable[TargetStatus[OLE]]) => Expr[TargetStatus[OLE]] = {
    if (p == checkA)
      ...
      val storeTargets = p.targetStores.flatMap(_.targetOwner).map(mkTargetId).map(_ -> loadTargets).toMap
      (c, loads) =>
        If(c === min[U]) Then (
          storeTargets.transform((k, v) => if (k == EDMARegister.id || ioServer.instruction.hostingTargets.map(_.id).contains(k)) loads.fmOf(ioServer.instruction) else Worst(v.map(t => loads.fmOf(t)).toSeq: _*)).toSeq
          ) Else (
      ...
      (c, loads) =>
        If(c === min[U]) Then (
          storeTargets.transform((k, v) => if (p.instruction.hostingTargets.map(_.id).contains(k)) loads.fmOf(p.instruction) else Worst(v.map(t => loads.fmOf(t)).toSeq: _*)).toSeq
          ) Else (
          storeTargets.transform((_, _) => c).toSeq
          )
    }
  ...
  /**
    ...
    */
  def softwareState(p: Application): (Variable[OLE], Variable[TargetStatus[OLE]]) => Expr[OLE] =
    if (p == checkA)
  ...
  val targetIsInputDep: Set[Target] = (DDR.banks ++ MSMC_SRAM.banks ++ corePacs.map(_.dsram)).toSet + EDMARegister
}
