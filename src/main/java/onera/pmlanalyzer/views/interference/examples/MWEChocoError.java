...
public class MWEChocoError {
     ...
     */
    private static void buildModelAndSolve(int k) {
        ...
        UndirectedGraph UB = new UndirectedGraph(model, nodeNames.length,
                SetType.BITSET, SetType.BITSET, false);
        for (int i = 0; i < nodeNames.length; i++) {
            UB.addNode(i);
        }
        for (String[] e : edges) {
        ...
        UndirectedGraph LB = new UndirectedGraph(model, nodeNames.length,
                SetType.BITSET, SetType.BITSET, false);
