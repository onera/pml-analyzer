...
/**
  ...
  *
  * @tparam T Possible modes of the automaton
  */
abstract class ModeAutomaton[T: IsCriticityOrdering : IsFinite] extends Component {
  ...
  def engage(e: Event): Unit = for (t <- fireable(e)) yield nextState = Some(t.computeNewState())
  ...
  def fireable(e: Event): Option[Transition[T]] = {
    transitions.filter(t => t.e == e && t.guard()) match {
      ...
      case s => throw new Exception(s"non deterministic automaton, $s are available for $e")
    }
  }
...
abstract class FMAutomaton[T: IsCriticityOrdering : IsFinite] extends ModeAutomaton[T] {
  val initialState: T
...
class SimpleFMAutomaton[T: IsCriticityOrdering : IsFinite] private(val id: AutomatonId, val initialState: T) extends FMAutomaton[T] {
  val events: Set[Event] = allWithNone[T].map(x => StochasticEvent(x.name, this)).toSet
  val eventMap: Map[Symbol, Event] = events.map(e => e.name -> e).toMap
  val transitions: Set[Transition[T]] =
    allWithNone[T].map(to =>
      Transition(
        () => getState < to,
        eventMap(to.name),
        () => to
  ...
  val o: OutputPort[T] = OutputPort(VariableId(Symbol("o")), () => Some(getState))
  val outputPorts: Set[OutputPort[T]] = Set(o)
}

object SimpleFMAutomaton {
  def apply[T: IsCriticityOrdering : IsFinite](id: AutomatonId, initialState: T)(implicit owner: Owner): SimpleFMAutomaton[T] = {
...
class InputFMAutomaton[T: IsCriticityOrdering : IsFinite : IsShadowOrdering](val id: AutomatonId, val initialState: T) extends FMAutomaton[T] {
  ...
  val transitions: Set[Transition[T]] =
    allWithNone[T].map(to =>
      ...
      Transition(
        () => (for (i <- in.eval()) yield i.inputShadow(getState) != getState) getOrElse false,
        epsilon,
...
object InputFMAutomaton {
  def apply[T: IsCriticityOrdering : IsFinite : IsShadowOrdering](id: AutomatonId, initialState: T)(implicit owner: Owner): InputFMAutomaton[T] = {
