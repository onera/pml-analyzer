...
trait PlatformCeciliaExporter {
  ...
  trait DependabilitySpecification {
    ...
    def softwareStoresDependency(p:PMLApplication) : (Variable[U], Variable[TargetStatus[U]]) => Expr[TargetStatus[U]]

    def softwareState(p:PMLApplication) : (Variable[U], Variable[TargetStatus[U]]) => Expr[U]
  ...
  private def platformIsExportable[
    FM : IsCriticityOrdering : IsFinite : IsShadowOrdering,
    T <: Platform with DependabilitySpecification.Aux[FM]: Typeable]: Aux[T, SystemModel] = new CeciliaExporter[T] {
    type R = SystemModel
    ...
    def toCecilia(x: T): SystemModel = {
      ...
      object TempSystem extends DepSystem(x.name) {
        ...
        def isReachable(from: Service, to: Service, in: Map[Service, Set[Service]]): Boolean =
          from == to || in.contains(from) && in(from).exists(succ => isReachable(succ, to, in))

        def getRouted(on: Service): (InitiatorId, TargetId) => Boolean = (i, t) => {
          val ini = x.initiators.filter { _.name == i.name }
          val tgt = x.targets.filter { _.name == t.name }
          ini.exists(pmlI => x.applications.filter(_.hostingInitiators.contains(pmlI)).exists(pmlS => tgt.exists(pmlT => pmlT.services.exists(tb => {
        ...
        val getAuthorized: (InitiatorId, TargetId) => Boolean = (i, t) => {
          val ini = x.initiators.filter { _.name == i.name }
          val s = x.applications.filter(_.hostingInitiators.intersect(ini).nonEmpty)
          val tgt = PLProvideService.domain.collect { case t2: PMLTarget if t.name == t2.name => t2 }
          s.exists(pmlS => tgt.exists(t => t.services.exists(b => {
        ...
        private val hwLinks = hwGraph.keySet.flatMap({ k => hwGraph(k) map { x => (k, x) } })
        private val hwComponents = hwLinks.flatMap({ p => Set(p._1, p._2) })

        // Transform targets
        private val targets = hwComponents.collect {
          case t: PMLTarget if targetIsInputDep.contains(t) => t -> InputDepTarget[FM](mkTargetId(t))
        ...
        val software: Map[PMLApplication, Software[FM]] =
          x.applications.collect {
            case a: PMLApplication =>
        ...
        //as rejection
        val transporters: Map[PMLTransporter, DepBasicTransporter[FM]] = hwComponents.collect {
          case t: PMLSimpleTransporter =>
            // add routing table to rejection
            val reject = (p: (InitiatorId, TargetId)) => t.services.exists(b => !getRouted(b)(p._1, p._2))
            t -> DepSimpleTransporter(TransporterId(t.name), reject)
          case v: Virtualizer =>
            // add routing table and authorize to rejection
            val reject = (p: (InitiatorId, TargetId)) => v.services.exists(b => !(getRouted(b)(p._1, p._2) && getAuthorized(p._1, p._2)))
            v -> DepVirtualizer(TransporterId(v.name), reject)
        }.toMap

        val initiators:Map[PMLInitiator, DepInitiator[FM]]= hwComponents.collect {
          case i: PMLInitiator =>
            i -> DepInitiator(InitiatorId(i.name))
        }.toMap

        // Compute inverse relation
        private val use = hwComponents.map(in => in -> hwLinks.collect {
          case (b, a) if a == in => b
        ...
        use.foreach {
          case (in: PMLTarget, outs) =>
            targets(in).storeI := outs.collect({
      ...
      val fSets = failureConditions.groupMap(p => p._3)(p =>
        (variablePathName(TempSystem,TempSystem.software(p._1).asInstanceOf[DepApplication[FM]].stateO),p._2.toString)
      )
      fSets.transform((k,v) => FailureConditions(v,k)).foreach(p => {
        val file = FileManager.exportDirectory.getFile(CeciliaExporter.ceciliaExportName(x) + s"FC${p._1}.xml")
