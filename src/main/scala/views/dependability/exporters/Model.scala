...
case class State(name: Symbol, tyype: CeciliaType, ini: String) {
  def toElem: Elem = tyype match {
    ...
    case tyype: RecordType => <alta.state name={name.name} refID={tyype.parent.id.toString} refPath={tyype.parent.absolutePath} value={ini}/>
  }
...
case class Flow(name: Symbol, tyype: CeciliaType, orientation: Orientation) {
  ...
  def toElem(x: Int, y: Int, id: Int): Elem = tyype match {
    case CeciliaBoolean =>
        <alta.flow name={name.name} type="bool" orientation={orientation.name} x={x.toString} y={y.toString} id={id.toString}/>
    case tyype: EnumeratedType =>
        <alta.flow name={name.name} refID={tyype.parent.id.toString} refPath={tyype.parent.absolutePath} orientation={orientation.name} x={x.toString} y={y.toString} id={id.toString}/>
  ...
  def toElem: Elem = tyype match {
    ...
    case tyype: EnumeratedType =>
        <alta.flow name={name.name} refID={tyype.parent.id.toString} refPath={tyype.parent.absolutePath} orientation={orientation.name}/>
    case tyype: RecordType =>
        <alta.flow name={name.name} refID={tyype.parent.id.toString} refPath={tyype.parent.absolutePath} orientation={orientation.name}/>
  }
}
...
object Model {
  ...
  trait ModelDescriptor[T] {
    ...
    private val subFamilies = collection.mutable.HashMap.empty[Path, SubFamilyFolder[T]]
    ...
    def getFolder(s: Symbol, parent: FamilyFolder[T]): SubFamilyFolder[T] =
      subFamilies.getOrElseUpdate(Paths.get(parent.absolutePath, s.name), new SubFamilyFolder(s, parent)(this))
  ...
  implicit object EnumeratedTypeDescriptor extends ModelDescriptor[EnumeratedType] {
  ...
  trait EquiBlockFlowPlacer extends FlowPlacer {
    ...
    private val _inputMargin = (icon.height - _basicSep * (_inputs.size - 1)) / 2
    private val _outputMargin = (icon.height - _basicSep * (_outputs.size - 1)) / 2
    ...
    def position(f: Flow): (Int, Int) = f match {
      ...
      case Flow(n, _, Out) =>
        if (_outputMargin > 0)
          (icon.width, _outputMargin + _outputs.indexWhere(_.name == n) * _basicSep)
        else
...
object EnumeratedType {
  def apply(name: Symbol,
            values: List[Symbol],
            parent: SubFamilyFolder[EnumeratedType])(implicit ev: ModelDescriptor[EnumeratedType]): EnumeratedType =
    ev.getModel(Paths.get(parent.absolutePath, name.name))(() => new EnumeratedType(name, values, VersionFolder(EntityFolder(name, parent))))
}
    ...
    <cec.model nature='type.structured' format='XML' encoding='UTF-8'>
      <cec.type.link type='undefine'>
        ...
        case Flow(n, tyype: EnumeratedType, _) =>
            <link.field name={n.name} refID={tyype.parent.id.toString} refPath={tyype.parent.absolutePath} direction="normal"/>
      })}
      </cec.type.link>
    </cec.model>
  }
...
object RecordType {
  def apply(name: Symbol,
            ...
            fields: List[Flow])(implicit ev: ModelDescriptor[RecordType]): RecordType =
    ev.getModel(Paths.get(parent.absolutePath, name.name))(() => new RecordType(name, VersionFolder(EntityFolder(name, parent)), fields.distinct))
}
...
object ComponentModel {
  val _components = collection.mutable.HashMap.empty[(Symbol,SubFamilyFolder[ComponentModel]), ComponentModel]
  def apply(name: Symbol,
            ...
            events: List[ConcreteEventModel],
            states: List[State],
            code: String): ComponentModel = {
    _components.getOrElseUpdate((name,parent),
    ComponentModel(
      name,
...
class EquipmentModel(
                      ...
                      val code: String) extends BlockModel[EquipmentModel] {
  parent.add(this)
  flows.foreach(_.add(this))
    ...
    <cec.model nature="equipment" format="XML" encoding="UTF-8">
      <cec.equipment width={icon.width.toString} height={icon.height.toString} auto-move="true" border="false">
        ...
        <alga.sheet id="1" name="Calque 1" visible="true" selected="true">
          {links.indices.map(i => <alga.link id={i.toString}/>)}{flows.map(f => <alga.port name={f.name.name} cx={position(f)._1.toString} cy={position(f)._2.toString} text.position='top'/>)}{subs.map(s => <alga.node name={s.name.name} cx={s.x.toString} cy={s.y.toString} text.position="top"/>)}
        </alga.sheet>
      </alga.view>
        <alta.code>{code}
        </alta.code>
...
object EquipmentModel {
  def apply(name: Symbol,
            ...
            code: String)(implicit ev: ModelDescriptor[EquipmentModel]): EquipmentModel = {
    val version = VersionFolder(EntityFolder(name, parent))
    ev.getModel(Paths.get(version.absolutePath))(() => new EquipmentModel(name, version, icon, simul, flows.distinct, subs.distinct, links, sync, code))
  }
}

class SystemModel(val name: Symbol,
                  val parent: VersionFolder[SystemModel],
                  val subs: List[SubComponent],
                  val links: List[(Flow, Flow)],
                  val sync: List[SynchroEventModel] = Nil,
                  val confs: List[Configuration] ,
                  val code: String) extends Model {
  parent.add(this)
  ...
  def toElem: Elem = {
    <cec.model nature="system.local" format="XML" encoding="UTF-8">
      <cec.mbsa>
        ...
        <alga.sheet id="1" name="Calque 1" visible="true" selected="true">
          {links.indices.map(i => <alga.link id={i.toString}/>)}{subs.map(s => <alga.node name={s.name.name} cx={s.x.toString} cy={s.y.toString} text.position="top"/>)}
        </alga.sheet>
      </alga.view>
...
object SystemModel {
  def apply(
             ...
             code: String)(implicit ev: ModelDescriptor[SystemModel]): SystemModel = {
    val version = VersionFolder(EntityFolder(name, parent))
    ev.getModel(Paths.get(version.absolutePath))(() => new SystemModel(name, version, subs.distinct, links, sync, conf, code))
  }
}
...
object ImageModel {
  def apply(parent: FamilyFolder[ImageModel], name: Symbol, stream: InputStream)(implicit m: ModelDescriptor[ImageModel]): ImageModel = {
    val version = VersionFolder(EntityFolder(name, parent))
    m.getModel(Paths.get(version.absolutePath))(() => new ImageModel(version, stream))
  }
  ...
  def toElem: Elem = {
    <cec.model nature="operator" format="XML" encoding="UTF-8">
      ...
      case Flow(_, tyype: EnumeratedType, _) =>
        <cec.operator refID={tyype.parent.id.toString} refPath={tyype.parent.absolutePath}>
          {inputs.map(_.toElem)}<alta.code>
          {code}
        </alta.code>
        </cec.operator>
...
object OperatorModel {
  def apply(parent: FamilyFolder[OperatorModel], inputs: List[Flow], output: Flow, code: String)(implicit ev: ModelDescriptor[OperatorModel]): OperatorModel = {
    val version = output.tyype match {
      case CeciliaBoolean =>
        VersionFolder(EntityFolder(output.name, SubFamilyFolder(CeciliaBoolean.name, parent)))
      case tyype: EnumeratedType =>
        VersionFolder(EntityFolder(output.name, SubFamilyFolder(tyype.name, parent)))
      case tyype: RecordType =>
        VersionFolder(EntityFolder(output.name, SubFamilyFolder(tyype.name, parent)))
    }
    ev.getModel(Paths.get(version.absolutePath))(() => new OperatorModel(version, inputs, output, code))
  }
}
...
case class FailureConditions(fc : Set[(String,String)], size:Int) extends Model {
  ...
  def toElem: Elem = {
    <targets outputType="file">
      {fc.map(p => <target var={p._1} val={p._2} path={new java.io.File(".").getCanonicalPath + File.separator + fileName(p._1,p._2)} order={size.toString}/>)}
    </targets>
  }
}