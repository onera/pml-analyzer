...
trait TargetCeciliaExporter {
  ...
  implicit def inputDepTargetExporter[FM: IsCriticityOrdering : IsFinite : IsShadowOrdering]: Aux[InputDepTarget[FM], EquipmentModel] = new CeciliaExporter[InputDepTarget[FM]] {
    ...
    def toCecilia(x: InputDepTarget[FM]): EquipmentModel = {
      ...
      val in = mkVariableName(x.storeI.id, tyype, In, x.storeI.eval().get.size)
      val out = Flow(x.loadO.id.name, tyype, Out)
      val obs = Flow(Symbol("storeO"),iTyype,Out)
      val direction = Flow(Symbol("direction"), typeModel[Direction.Value], Out)
      ...
      val (worstFlow, worstAssertions, worstSub) = mkWorstSub(myFields.flatMap{f => in.map(i => s"$i^$f")}.toSet)
      val inAssertion =
      ...
      val worstByIni = allOf[InitiatorId].map(f => f -> mkWorstSub(in.map(i => s"$i^${(f,x.id).name.name}").toSet)).toMap
      val obsSub =
        allOf[InitiatorId].map(f => f ->
          mkContainerShadowSub(Symbol(s"shadowStore$f"),s"${worstByIni(f)._1}",s"$automaton.${x.fMAutomaton.o}")).toMap
      EquipmentModel(
        x.id.name,
        phylogTargetFolder,
        phylogBlockBlue,
        ...
        obsSub.values.map(_._3).toList ++ worstByIni.values.map(_._3) :+ automaton :+ worstSub,
        Nil,
        Nil,
        s"""assert
           ...
           |${worstByIni.values.flatMap(_._2).toList.distinct.sorted.mkString("\n")}
           |${obsSub.values.flatMap(_._2).toList.distinct.sorted.mkString("\n")}
           |
           |// observator of store states according to component state
           |${obsSub.map(p => s"$obs^${p._1} = ${p._2._1};").toList.sorted.mkString("\n")}
           |
           |// icone management
  ...
  implicit def inputInDepTargetIsExportable[FM: IsCriticityOrdering : IsFinite : IsShadowOrdering]: Aux[InputInDepTarget[FM], EquipmentModel] = new CeciliaExporter[InputInDepTarget[FM]] {
    type R = EquipmentModel
    ...
    def toCecilia(x: InputInDepTarget[FM]): EquipmentModel = {
      ...
      val outAssertions = myFields.map(f => s"$out^$f = $sub.${x.fMAutomaton.o.id};") ++
        otherFields.map(f => s"$out^$f = ${noneOf[FM].name.name};")
      val obsSub =
        allOf[InitiatorId].map(f => f ->
          mkContainerShadowSub(Symbol(s"shadowStore$f"),s"$in^${(f,x.id).name.name}",s"$sub.${x.fMAutomaton.o}")).toMap
