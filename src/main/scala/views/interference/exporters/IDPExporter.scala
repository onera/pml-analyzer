...
object IDPExporter {
  ...
  trait Ops {
    ...
    implicit class IdpExporterOps(platform: Platform with InterferenceSpecification) {
      def exportAsIDP()(implicit exporter: IDPPlatformExporter.type ): Unit = {
        val startDate = System.currentTimeMillis()
        val writer = new FileWriter(FileManager.exportDirectory.getFile(platform.fullName + ".pia"))
        exporter.exportIDP(platform)(writer)
        writer.close()
        println(Message.successfulExportInfo(platform.fullName, System.currentTimeMillis() - startDate))
      }
    }
  ...
  implicit object IDPPlatformExporter {
    ...
    def pathId(p:PhysicalTransaction):String = _memoPathId.getOrElseUpdate(p,{
      val sameHT = _memoPathId.keys.count(k => k.head == p.head && k.last == p.last)
      ...
      lazy val _paths: ((A, Set[A])) => Set[Path[A]] = immutableHashMapMemo {
        s =>
          if(s._2.contains(s._1) ) {
            ...
            graph(s._1).flatMap(next => _paths((next, s._2 + s._1)) map {next +: _} )
      }
    ...
    def exportIDP(platform: Platform with InterferenceSpecification)(implicit writer: Writer): Unit = {
      import platform._
      ...
      val nopServices = hwComponents.collect { case i: Initiator => i.name.name -> s"${i.name.name}_NOP" }
      ...
      writer.write((services.map { s => s"${s.name.name} -> ${s.typeName.name.toUpperCase}" } ++
        nopServices.map(kv => s"${kv._2} -> NOP")).toList.sorted.mkString("ServiceType = {\n\t", ";\n\t", "\n}\n\n"))

      writer.write("/* Provider of used services */\n")
      writer.write((services.flatMap { s => PLProvideService.inverse(s) map { owner => s"${s.name.name} -> ${owner.name.name}" } } ++
        nopServices.map(kv => s"${kv._2} -> ${kv._1}")).toList.sorted.mkString("ProvidedBy = {\n\t", ";\n\t", "\n}\n\n"))

      val pathNames = platform match {
        case l:TransactionLibrary =>
          val names = l.transactionUserName
          transactions.keySet.map { k => k -> {if(names(k).isEmpty) Set(s"${k}_path") else names(k).map(t => s"${t}_path") }}.toMap
        case _ => transactions.keySet.map { k => k -> Set(s"${k}_path") }.toMap
      }

      writer.write("/* Services paths used by at least one transaction */\n")
      writer.write((pathNames.values.flatten ++
        nopServices.map(kv => s"${kv._2}_${kv._2}_path")).toList.sorted.mkString("Paths = {\n\t", ";\n\t", "\n}\n\n"))

      // the successors of a nop transaction is always nop
      val nopSuccessor = nopServices.map(kv => s"${kv._2}_${kv._2}_path, ${kv._2} -> ${kv._2}")

      writer.write("/* Successor relation for paths */\n")
      writer.write((pathNames.transform {(k,v) =>  v flatMap  {name => transactions(k).sliding(2) map {l => s"$name, ${l.head} -> ${l.last}"} }}.values.flatten ++
        nopSuccessor).toList.sorted.mkString("NextPathService = {\n\t", ";\n\t", "\n}\n\n"))

      val transactionNames = platform match {
        case l:TransactionLibrary =>
          val names = l.transactionUserName
          transactions.keySet.map { k => k -> {if(names(k).isEmpty) Set(k.id.name) else names(k).map(_.id.name) }}.toMap
        case _ => transactions.keySet.map { k => k -> Set(k.id.name) }.toMap
      }

      writer.write("/* Single transactions triggered by software */\n")
      writer.write((transactionNames.values.flatten ++
        nopServices.map(kv => s"${kv._2}_${kv._2}")).toList.sorted.mkString("SingleTransactions = {\n\t", ";\n\t", "\n}\n\n"))

      writer.write("/* Head of service path of the single transaction */\n")
      writer.write((transactionNames.transform {(k,v) =>  v map {name => s"$name -> ${transactions(k).head.name.name}"} }.values.flatten ++
        nopServices.map {kv => s"${kv._2}_${kv._2} -> ${kv._2}"}).toList.sorted.mkString("TransactionInitiatorService = {\n\t", ";\n\t", "\n}\n\n"))

      writer.write("/* Target service of the single transaction */\n")
      writer.write((transactionNames.transform {(k,v) =>  v map {name => s"$name -> ${transactions(k).last.name.name}"} }.values.flatten  ++
        nopServices.map {kv => s"${kv._2}_${kv._2} -> ${kv._2}"}).toList.sorted.mkString("TransactionTargetService = {\n\t", ";\n\t", "\n}\n\n"))

      writer.write("/* Path of the single transaction */\n")
      writer.write((transactions.keySet.flatMap { k => transactionNames(k) flatMap { trName => pathNames(k) map { pName => s"$trName -> $pName" }}} ++
        nopServices.map {kv => s"${kv._2}_${kv._2} -> ${kv._2}_${kv._2}_path"}).toList.sorted.mkString("TransactionPath = {\n\t", ";\n\t", "\n}\n\n"))
      ...
      writer.write(
        s"""
          ...
          |TransparentTransactions = ${transactions
          .keySet
          ...
          |TransactionAffect = ${transactions
          .keySet
          .flatMap { k => platform.transactionInterfereWith(k).flatMap(s => transactionNames(k) map { name => s"$name,$s"})}
          .toList
          .sorted
          .mkString("{\n\t",";\n\t","\n}\n\n")}
          |
          ...
          |TransactionNotAffect  = ${transactions
          .keySet
          .flatMap(k => platform.transactionNotInterfereWith(k).flatMap(s =>  transactionNames(k) map { name => s"$name,$s"}))
          .toList
          .sorted
          .mkString("{\n\t",";\n\t","\n}\n\n")}
          |
          ...
          .subsets(2)
          .collect {case si if platform.finalInterfereWith(si.head,si.last) => si.mkString(",")}
          .toList
          .sorted
          .mkString("{\n\t",";\n\t","\n}\n\n")}
          |
          |// Two transactions "t" and "t'" cannot be triggered simultaneously
          |TransactionsAreExclusive  = ${transactions
          .keySet
          .subsets(2)
          .filter {s => platform.finalExclusive(s.head,s.last)}
          .flatMap(s => transactionNames(s.head) flatMap {leftName => transactionNames(s.last) map { rightName => s"$rightName, $leftName"}})
          .toList
          .sorted
          .mkString("{\n\t",";\n\t","\n}\n\n")}
          |
          ...
          |Parallel = ${services
          .subsets(2)
          .filter {ss => ss.head.hardwareOwner.intersect(ss.last.hardwareOwner).nonEmpty && !platform.finalInterfereWith(ss.head,ss.last)}
          .map {ss => ss.mkString(",")}
          .toList
          .sorted
          .mkString("{\n\t",";\n\t","\n}\n\n")}
