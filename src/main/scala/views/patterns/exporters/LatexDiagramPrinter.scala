  ...
  implicit class LatexDiagramPrinterConclusion(conclusion: Claim) {
    ...
    val file: File =  {
      for(s <- conclusion.short) yield
        FileManager.exportDirectory.getFile(s"${s}_diagram.tex")
    ...
    private def writeString(s:String)(implicit printer: FileWriter, spacing:Int) : Unit ={
      s.split("\n").foreach { chunk =>
        addSpacing
        printer.write(chunk + "\n")
      }
    ...
    private def print(claim: Claim)(implicit printer: FileWriter, spacing:Int): Unit = {
      writeString(s"[{${ids(claim)} ${claim.label + getImplementation(claim.implementation)}}, conclusion,  s sep-= 1.25em, calign=first, ${getSize(claim.textWidth)} \n")
    ...
    private def print(strategy: Strategy)(implicit printer: FileWriter, spacing:Int): Unit = {
      writeString(s"[{${ids(strategy)} ${strategy.label} ${getImplementation(strategy.implementation)}}, strategy, ${getSize(strategy.textWidth)}\n")
    }
    ...
    private def print(evidence: Evidence)(implicit printer: FileWriter, spacing:Int): Unit = evidence match {
      case e@FinalEvidence(content, implementation, textWidth, None) =>
        writeString(s"[{${ids(e)} $content ${getImplementation(implementation)}}, conclusion, ${getSize(textWidth)}] \n")
      case e@FinalEvidence(content, implementation, textWidth, Some(refOf)) =>
        val refPrinter = LatexDiagramPrinterConclusion(refOf)
        val refPath = Paths.get(refPrinter.file.getName).toString.replace(".tex",".pdf")
        refPrinter.printDiagram()
        writeString(s"[{\\href{$refPath}{${ids(e)}} $content  ${getImplementation(implementation)}}, conclusion, ${getSize(textWidth)}] \n")
      case g@Given(content, implementation, textWidth) =>
        writeString(s"[{${ids(g)} $content  ${getImplementation(implementation)}}, conclusion,  ${getSize(textWidth)}, dashed ] \n")
      case c: Claim =>
        print(c)
    }
  }
