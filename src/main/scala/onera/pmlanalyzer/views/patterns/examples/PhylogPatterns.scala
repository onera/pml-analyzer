...
object PhylogPatterns extends App {
  ...
  val alterationIdentified =
    conclusion(
      "All critical and innocuous configuration settings alterations $\\mathcal{A}lt$ identified (no case missing)"
  ...
  val alterationMitigated =
    conclusion(
      "Design of adequate mitigation means against inadvertent changes to Configuration Settings" size 8
    ).strategy(
      "Check all identified settings alterations are mitigated ($\\forall a_i \\in \\mathcal{A}lt$ $a_i$ mitigated)"
  ...
  val RU2 =
    conclusion(
      ...
      "DO178: Implementation compliant with specification \\\\ DO254: Hardware usage compliant with specification" size 11
    ).evidence(alterationMitigated)
      .`given`(
        "Mitigation means implementation verified \\\\ (DO178B/C compliance)"
      )
  ...
  val platformAnalysis =
    conclusion("Safety objectives $\\mathcal{R}eq$ are fulfilled")
      ...
      .`given`(
        "Safety objectives $\\mathcal{R}eq$,\\\\ configuration settings \\\\and application mapping"
      )
      .`given`("Mitigation means of RU2, RU3 and (E7)")
      .`given`("Critical alteration from RU2")

  val equipmentAnalysis = conclusion(
    "Errors contained within equipment" short "equipmentAnalysis" size 8
  ...
  val failure_mode_mitigated =
    conclusion("Design of adequate mitigation means")
      .strategy(
        "Traceability analysis: \\\\ Check all identified failure modes are mitigated" size 6
      )
      .evidence(failure_mode_identified)
      .evidence("$fm_i$ is mitigated")

  val implementationCompliant = conclusion(
    "Designed, implemented and verified adequate mitigation means for identified failure modes" short "implementationCompliant" size 8
  ).strategy(
    "DO178: Implementation compliant with specification \\\\ DO254: Hardware usage compliant with specification" size 8
  ...
  val EH =
    conclusion(
      "Mitigation means commensurate with the safety objectives" short "EH" width patternWidth size 8
    ).strategy(
      "Check matching between equipment safety analysis and mitigation means implementation"
  ...
  val interference_identified_classified =
    conclusion(
      "Classification of interference effects $( \\forall i \\in \\mathcal{I}, c(i))$"
    ).strategy("Safety analysis" backing "Architecture mastery")
      .evidence(interference_identification)
      .evidence("Identification of $i$ effects")
      .`given`(
        "Configuration settings $\\mathcal{C}s$ and application mapping and temporal constraints on applications (e.g. WCET)"
      )

  val identified_mitigated =
    conclusion("Design of adequate means of mitigation for interference")
      .strategy(
        "Check all identified interference are mitigated ($\\forall i \\in \\mathcal{I}$, $i$ mitigated)"
      )
      .evidence(interference_identified_classified)
      .evidence(
        "$i$ mitigated\\\\(e.g. prevention / blocking with run-time mechanism; impossible due to usage domain restriction; tolerance)"
      )

  val RU3 =
    conclusion(
      "Identification of interference and verified means of mitigation" short "RU3" width patternWidth
    ).strategy(
      "DO178: Implementation compliant with specification \\\\ DO254: Hardware usage compliant with specification" size 11
  ...
  val interference_identified_classified_short =
    conclusion(
      ...
      .evidence(
        "Identification of all interference $\\mathcal{I}$ \\\\ Given: Configuration settings $\\mathcal{C}s$"
      )
      .evidence("Identification of $i$ effects")
      .`given`(
        "Configuration settings $\\mathcal{C}s$ and application mapping and temporal constraints on applications (e.g. WCET)"
  ...
  val RU3_short =
    conclusion(
      "Identification of interference and verified means of mitigation" short "RU3Short" width patternWidth
    ).strategy(
      "DO178: Implementation compliant with specification \\\\ DO254: Hardware usage compliant with specification" size 11
  ...
  val software_demand_identified =
    conclusion("Identify demand of all software")
      ...
      .`given`(
        "- Execution model \\\\- Final configuration settings \\\\- Software implementation"
      )
  ...
  val hardware_capacity_identified =
    conclusion("HW resources with their capacities identified")
      .strategy(
        "Reading and stressing benchmarks" backing "architecture mastery"
      )
      .`given`("MCP resources")
  ...
  val RU4 =
    conclusion(
      "Identify the available MCP resources, how they are allocated and verify demand does not exceed the amount of resources available "
        short "RU4"
        size 10
        width patternWidth
  ...
  val configuration_mitigation = conclusion(
    "Configuration settings $i$ contributes to at least one requirement"
  ...
  val correct_config_spec =
    conclusion(
      "Configuration settings specification enables the hardware and the software to satisfy the requirements"
    ).strategy(
      "Check contribution of each configuration settings $(\\forall i \\in \\mathcal{C}s)$ to the requirements"
    ).evidence("Description of the configuration settings $\\mathcal{C}s$")
      .evidence(configuration_mitigation)
      .evidence(
        "Configuration settings $i$ not contributing to requirements is innocuous"
      )
  ...
  val RU1 = conclusion(
    ...
    .`given`(
      "Configuration settings means implementation verified \\\\ (DO178B/C compliance)"
    )
    .`given`("Hardware configuration verified \\\\ (DO254 compliance)")
