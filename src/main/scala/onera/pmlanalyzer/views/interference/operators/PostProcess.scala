...
object PostProcess {
  ...
  trait Ops {
    ...
    extension[T](self:T) {
      ...
      /**
        * Compare the interference results with another element and store the result in a dedicated file in the
        * analysis folder
        * @param that the other element
        * @param ev the proof that any element of T is analysable
        * @return the location of the result files
        */
      def interferenceDiff(that: Platform)(using ev:PostProcess[T]): Seq[File] = ev.interferenceDiff(self,that)

      /**
        * Try to find and parse itf results for the considered element
        * @param ev the proof that any element of T is analysable
        * @return the set of set of scenario identifiers that are interference scenarios
        */
      def parseITFScenarioFile()(using ev:PostProcess[T]): Array[Seq[String]] = ev.parseITFScenarioFile(self)

      /**
        * Try to find and parse the n-itf results for the considered element
        * @param n the maximal size of transaction per itf scenario
        * @param ev the proof that any element of T is analysable
        * @return the set of set of scenario identifiers that are interference scenarios
        */
      def parseITFScenarioFile(n: Int)(using ev:PostProcess[T]): Array[Seq[String]] = ev.parseITFScenarioFile(self,n)

      /**
        * Try to find and parse the n-itf-free results for the considered element
        * @param n the maximal size of transaction per itf scenario
        * @param ev the proof that any element of T is analysable
        * @return the set of set of scenario identifiers that are interference free scenarios
        */
      def parseFreeScenarioFile(n: Int)(using ev:PostProcess[T]): Array[Seq[String]] = ev.parseFreeScenarioFile(self, n)
      ...
      /**
        ...
        * The result is provided in a file of the analysis directory
        * @param max the optional maximal size of the considered itf
        * @param ev the proof that any element of T is analysable
        * @return the location of the result files
        */
  ...
  given PostProcess[ConfiguredPlatform] with {
    ...
    def interferenceDiff(x: ConfiguredPlatform, that: Platform): Seq[File] = {
      for {
        ...
        thisITFFile <- FileManager.analysisDirectory.locate(s"${x.fullName}_itf_$size.txt")
        thatITFFile <- FileManager.analysisDirectory.locate(s"${that.fullName}_itf_$size.txt")
      } yield {
        val file = FileManager.analysisDirectory.getFile(s"${x.fullName}_diff_${that.fullName}_itf_$size.txt")
        ...
        if (thisDiffThat.isEmpty)
          ...
          writer.write(s"The following ${thisDiffThat.length} itf of ${x.fullName} are not in ${that.fullName}\n")
          thisDiffThat.foreach(d => writer.write(s"$d\n"))
        }
        if (thatDiffThis.isEmpty)
          writer.write(s"All itf of ${that.fullName} are in ${x.fullName}\n")
        else {
          writer.write(s"The following ${thatDiffThis.length} itf in ${that.fullName} are not in ${x.fullName}\n")
          thatDiffThis.foreach(d => writer.write(s"$d\n"))
        }
        writer.flush()
        writer.close()
        println(Message.successfulITFDifferenceExportInfo(size, x.fullName, that.fullName, file.getAbsolutePath))
        file
      }
    }
    ...
    def parseITFScenarioFile(x:ConfiguredPlatform, n: Int): Array[Seq[String]] = {
      for {
        file <- FileManager.analysisDirectory.locate(s"${x.fullName}_itf_$n.txt")
      } yield {
    ...
    def parseFreeScenarioFile(x:ConfiguredPlatform, n: Int): Array[Seq[String]] = {
      for {
        file <- FileManager.analysisDirectory.locate(s"${x.fullName}_free_$n.txt")
      } yield {
        val s = Source.fromFile(file)
        val result = parseScenarioFile(s)
        s.close()
    ...
    def sortPLByITFImpact(x: ConfiguredPlatform, max: Option[Int]): Future[Set[File]] = {
      x.computeKInterference(max.getOrElse(x.initiators.size), ignoreExistingAnalysisFiles = false, verboseResultFile = false) map {
        resultFiles =>
          resultFiles.filter(_.getName.contains("channel")).map(resultFile => {
            val size = resultFile.getName.split("\\D+").filter(_.nonEmpty).last.toInt
            val file = FileManager.analysisDirectory.getFile(s"${x.fullName}_HW_importance_factor_itf_$size.txt")
            val writer = new FileWriter(file)
            writer.write(PLInvolvedInITF(x, resultFile)
              .toSeq
              .sortBy(-_._2)
              .map(p => s"${p._1},${p._2}")
              .mkString("\n"))
    ...
    private def PLInvolvedInITF(x:ConfiguredPlatform, file: File): Map[Hardware, Int] = {
      import x._
      val results = parseChannel(Source.fromFile(file))
      val plByScenario = results
        .map(p => p._1.flatMap(s => x.directHardware.filter(_.services.exists(s2 => s2.toString == s))).toSet -> p._2)
      ...
      x.computeKInterference(max.getOrElse(x.initiators.size), ignoreExistingAnalysisFiles = false, verboseResultFile = false) map {
        resultFiles => {
          val multiPathsTransactions = x match {
            ...
            case _ =>
              x.multiPathsTransactions.map(_.map(x.transactionsName).map(_.toString))
          }
          resultFiles.filter(_.getName.contains("itf")).map(resultFile => {
            val size = resultFile.getName.split("\\D+").filter(_.nonEmpty).last.toInt
            val file = FileManager.analysisDirectory.getFile(s"${x.fullName}_Routing_importance_factor_itf_$size.txt")
            val writer = new FileWriter(file)
            writer.write({
              val source = Source.fromFile(file)
              val itf = parseScenarioFile(source)
              source.close()
              multiPathsTransactions.groupMapReduce(k => k)(k =>
                itf.count(sc => sc.exists(s => k.contains(s))))(_ + _)
                .toSeq
                .sortBy(-_._2)
    ...
    def sortSWByITFImpact(x: ConfiguredPlatform, max: Option[Int]): Future[File] = {
      x.computeKInterference(max.getOrElse(x.initiators.size), ignoreExistingAnalysisFiles = false, verboseResultFile = false) map {
        resultFiles => {
          val file = FileManager.analysisDirectory.getFile(s"${x.fullName}_SW_importance_factor.txt")
          val writer = new FileWriter(file)
          writer.write(resultFiles.filter(_.getName.contains("itf"))
    ...
    private def SWInvolvedInITF(x:ConfiguredPlatform, file: File): Map[Set[Application], Int] = {
      val results = parseScenarioFile(Source.fromFile(file))
      val swByScenario = results
        .map(_.flatMap(s => x.applications.filter(sw => x.transactionsBySW(sw).exists(s2 => s2.toString == s))).toSet)
