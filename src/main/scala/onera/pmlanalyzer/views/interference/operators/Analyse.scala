...
trait Analyse[-T] {
  def computeInterference(
                           ...
                           verboseResultFile: Boolean): Future[Set[File]]
...
object Analyse {
    ...
    /**
     ...
     extension[T<: Platform](self:T) {
      ...
      /**
        ...
        * @param maxSize the maximal number of initiator that can execute a transaction,
        *                it should be less or equal to the total number of initiators in the platform.
        * @param ignoreExistingAnalysisFiles do the analysis only even if result files for the considered platform
        *                                    can be found in the analysis directory
        * @param verboseResultFile add extra information on analysis files
        * @param ev the proof that the component is analysable
        * @return the computation future
        */
       def computeKInterference(maxSize: Int,
                                ignoreExistingAnalysisFiles: Boolean,
                                verboseResultFile: Boolean)(using ev:Analyse[T]): Future[Set[File]] =
         ev.computeInterference(self,maxSize,ignoreExistingAnalysisFiles,verboseResultFile)

      /**
        * Perform the interference analysis
        * @param maxSize the maximal number of initiator that can execute a transaction,
        *                it should be less or equal to the total number of initiators in the platform.
        * @param timeout the maximal duration that is allowed to perform the interference computation.
        * @param ignoreExistingAnalysisFiles do the analysis only even if result files for the considered platform
        *                                    can be found in the analysis directory (false by default)
        * @param verboseResultFile add extra information on analysis files (false by default)
        * @param ev the proof that the component is analysable
        * @return the computation future
        */
       def computeKInterference(maxSize: Int,
         ...
         Await.result(ev.computeInterference(self, maxSize, ignoreExistingAnalysisFiles, verboseResultFile),timeout)
      ...
      /**
        ...
        * @return the computation future
        */
       def computeAllInterference(timeout: Duration,
                                  ignoreExistingAnalysisFiles: Boolean = false,
                                  verboseResultFile: Boolean = false
       ...
       def getAnanlysisGraphSize()(using ev:Analyse[T]): (BigInt, BigInt) = ev.getGraphSize(self)
     }
   }
  ...
  given Analyse[ConfiguredPlatform] with {
    ...
    def printGraph(platform: ConfiguredPlatform): File = {
      val problem = computeProblemConstraints(platform, platform.initiators.size)
      val result = FileManager.exportDirectory.getFile(s"${platform.name.name}_graph.dot")
    ...
    def computeInterference(
      ...
      val interferenceFiles = sizes.map(size => size -> FileManager.analysisDirectory.getFile(s"${platform.fullName}_itf_$size.txt")).toMap
      val freeFiles = sizes.map(size => size -> FileManager.analysisDirectory.getFile(s"${platform.fullName}_free_$size.txt")).toMap
      val channelFiles = sizes.map(size => size -> FileManager.analysisDirectory.getFile(s"${platform.fullName}_channel_$size.txt")).toMap
      val files = (interferenceFiles.values ++ freeFiles.values ++ channelFiles.values).toSet
      if (!ignoreExistingAnalysisFiles && files.forall(f => FileManager.analysisDirectory.locate(f.getName).isDefined)) {
        println(Message.analysisResultFoundInfo(FileManager.analysisDirectory.name, platform.fullName))
        files
      } else {
        val generateModelStart = System.currentTimeMillis()
        val problem = computeProblemConstraints(platform, maxSize)
        val summaryFile = FileManager.analysisDirectory.getFile(s"${platform.fullName}_itf_calculus_summary.txt")
        val summaryWriter = new FileWriter(summaryFile)
        val interferenceWriters = interferenceFiles.transform((_, v) => new FileWriter(v))
        val freeWriters = freeFiles.transform((_, v) => new FileWriter(v))
        val channelWriters = channelFiles.transform((_, v) => new FileWriter(v))
        val allWriters = channelWriters.values ++ freeWriters.values ++ interferenceWriters.values
        ...
        val update = (isFree: Boolean,
                      physical: Set[Set[PhysicalScenarioId]],
                      user: Map[Set[PhysicalScenarioId], Set[Set[UserScenarioId]]]) => {
          val userBySize = user.values.flatten.groupBy(_.size).transform((_, v) => v.toSet)
          if (isFree) {
        ...
        println(Message.successfulModelBuildInfo(platform.fullName, (System.currentTimeMillis().toFloat - generateModelStart) * 1E-3))
        ...
        for {(k, v) <- problem.litToNodeSet
             ...
             userDefined = physical.groupMapReduce(p => p)(problem.decodeUserModel)(_ ++ _)}
        ...
        println(Message.iterationResultsInfo(isFree = false, nbITF, nonExclusiveScenarios))
        println(Message.iterationResultsInfo(isFree = true, nbFree, nonExclusiveScenarios))

        for (size <- sizes) {
          val iterationStartDate = System.currentTimeMillis()
          val s = problem.instantiate(size)
          val variables = problem.groupedScenarios.transform((k, _) => k.toLit(s))
          while (s.solve()) {
            val cube = variables.filter(_._2.value())
            val physical = problem.decodeModel(cube.keySet, problem.isFree.toLit(s).value())
        ...
        val computationTime = (System.currentTimeMillis() - assessmentStartDate) * 1E-3
        ...
        summaryWriter.write(Message.printScenarioNumber(nbITF, nonExclusiveScenarios))
    ...
    private def undirectedEdgeId(l: MNode, r: MNode): EdgeId = Symbol(List(l, r).map(_.id.name).sorted.mkString("--"))
    ...
    private def computeProblemConstraints(
      ...
      val initialServicesInterfere = initialServices
        .map(s => s -> initialServices.filter(s2 => platform.finalInterfereWith(s, s2)))
        .toMap
            ...
            transactions(t).filter(s =>
              transactions.keySet.exists(t2 =>
                t != t2 &&
                  ...
                  transactions(t2).exists(s2 => s2 == s || initialServicesInterfere(s2).contains(s))))))
        .toMap

      val services = pathT.values.toSet.flatten.flatten

      val servicesExclusive = services.map(s => s -> services.filter(s2 => platform.finalInterfereWith(s, s2))).toMap
      ...
      val nodeToServices = serviceToNodes
        .keySet
      ...
      val scenarioToGroupedLit = reducedNodePath
        ...
        .groupMapReduce(_._1)(x => addLit(groupedScenarioLitId(x._2.toSet).id))((l, _) => l)
      ...
      val groupedLitToNodeSet = scenarioToGroupedLit
        .groupMapReduce(_._2)(kv => reducedNodePath(kv._1).map(_.toSet.flatten))((l, _) => l)

      val addUndirectedEdgeI: Set[MNode] => MEdge = immutableHashMapMemo {
        lr => MEdge(lr.head, lr.last, undirectedEdgeId(lr.head, lr.last))
      }

      val addUndirectedEdge = (lr: Set[Service]) =>
        serviceToNodes(lr.head).flatMap(n1 => serviceToNodes(lr.last).map(n2 => addUndirectedEdgeI(Set(n1, n2))))
      ...
      val edgesToScenarios: Map[MEdge, Set[PhysicalScenarioId]] = pathT
        .keySet
        .flatMap(s =>
          pathT(s)
            .filter(_.size > 1)
            .flatMap(t =>
              t.sliding(2).flatMap(slice => addUndirectedEdge(slice.toSet).map(_ -> s))
            )
        )
        .groupMap(_._1)(_._2)
      ...
      val edgeCst = edgesToScenarios
        .transform((k, trs) => SimpleAssert(Equal(MEdgeLit(k, graph), Or(trs.map(scenarioToGroupedLit).toSeq: _*))))
      ...
      val (trivialFreeScenarios, otherScenarios) = scenarioToGroupedLit
        .values
        .toSet
        .partition(groupedLitToNodeSet(_).forall(_.isEmpty)) //All paths of the scenarios are only private ones
      ...
      val isITF = And(
        (trivialFreeScenarios.map(v => Not(v))
          ...
          .map(ss => {
            ...
            Implies(And(vs, vsp), Or(sHeads.flatMap(head => spLasts.map(last => Reaches(graph, head, last))).toSeq: _*))
          })).toSeq: _*)

      val isFree = And(
        otherScenariosCouples.map(ss => {
          val (vs -> sHeads, vsp -> spLasts) = ss
          Implies(And(vs, vsp), Not(Or(sHeads.flatMap(head => spLasts.map(last => Reaches(graph, head, last))).toSeq: _*)))
        }).toSeq: _*)

      //for each scenario, the scenarios that are exclusive with it
      val exclusiveScenarios = idToScenario
        .transform((k, _) => idToScenario.keySet.filter(kp => k != kp && platform.finalExclusive(k, kp)))
      ...
      val onSnPerGrouped = groupedLitToScenarios.transform((k, v) => Implies(k, Or(v.map(scenarioToLit).toSeq: _*)))
      val nonExclusiveSn = scenarioToLit.transform((k, v) => exclusiveScenarios(k).map(scenarioToLit).map(sLit => Implies(sLit, Not(v))))

      val nonExclusive = (onSnPerGrouped.values ++ nonExclusiveSn.values.flatten).map(SimpleAssert.apply)

      val serviceToScenarioLit = idToScenario
        .keySet
        .flatMap(k => idToScenario(k).flatMap(tr => transactions(tr).map(_ -> k)))
    ...
    private def writeFooter(writer: FileWriter, computationTime: Double, size: Int = -1): Unit =
      writer write
        s"""------------------------------------------
    ...
    private def writeFileInfo(writer: FileWriter, platform: Platform & InterferenceSpecification): Unit =
      writer write
    ...
    private def updateChannelFile(writer: Map[Int, FileWriter], channels: mutable.Map[Int, Map[Channel, Int]]): Unit = {
      for ((k, v) <- channels)
        writer(k)
          .write(v.map(c => s"${channelId(c._1)}: ${c._2} interferences").toList.sorted.mkString("\n"))
    }
    ...
    private def updateChannelNumber(
      ...
      for ((k, v) <- channelNb)
        channels.get(k) match {
          case Some(map) =>
            channels(k) = (map.toSeq ++ v.toSeq).groupMapReduce(_._1)(_._2)(_ + _)
          case None =>
            channels(k) = v
        }
    }
