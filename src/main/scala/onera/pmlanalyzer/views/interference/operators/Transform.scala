...
object Transform {
  ...
  trait InterferenceSpecificationInstances {
    self: InterferenceSpecification & Platform =>

    /** Convert a physical scenario id to the set of its physical transactions ids
     *
     * @group transform_operator
     */
    given Transform[PhysicalScenarioId, Set[PhysicalTransactionId]] with {
      def apply(l: PhysicalScenarioId): Set[PhysicalTransactionId] =
        purifiedScenarios.getOrElse(l, Set.empty)
    }
  }
}
