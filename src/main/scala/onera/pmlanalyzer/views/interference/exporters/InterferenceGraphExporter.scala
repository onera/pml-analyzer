    ...
    implicit class InterferenceGraphExporterOps(x: Platform with InterferenceSpecification) extends UMLExporter.Ops {
      def exportGraph(it: Set[PhysicalScenarioId]): Unit = {
        ...
        for {s <- services.subsets(2) if x.finalInterfereWith(s.head, s.last)}
          DOTServiceOnly.writeAssociation(DOTServiceOnly.getId(s.head).get, DOTServiceOnly.getId(s.last).get, "exclusive")

        for {s <- it
             t <- x.purifiedScenarios(s)
             l = x.purifiedTransactions(t).sliding(2).collect { case Seq(f, t) => f -> t }.toList
             if l.nonEmpty} {
          val transaction = s"""${t.id.name}[label = "{${t.id.name} : Transaction}"]"""
          writer.write(s"$transaction\n")
          DOTServiceOnly.writeAssociation(t.id.name, DOTServiceOnly.getId(l.head._1).get)
