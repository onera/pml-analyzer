...
/**
  ...
  * @tparam MyBDD the BDD representation which must be a subtype of JavaBDD type
  */
trait BaseBDDFactory[Var, MyBDD <: JavaBDD] {

  /**
    * initialisation of the cache and number of node of the factory
    * @param numberOfVar maximum number of variables in BDDs
    * @param cacheSize initial size of the cache table containing BDD nodes
    * @return
    */
  ...
  /**
    ...
    * @param i the index of the BDD node
    * @return BDD node
    */
  ...
  /**
    ...
    * @param vs the ordered sequence of variables
  ...
  def mkExactlyK(vs:Seq[Var], k:Int): MyBDD = {
    ...
    def BDDFromTable(of: (Int, Int), in: Map[(Int, Int), (Var, (Int, Int), (Int, Int))]): MyBDD = _memo.getOrElseUpdate(of,
      if (of == oneIdx)
        one()
      else if (of == zeroIdx)
        zero()
    ...
    if (k == 0 || ds.size < k)
      ...
      val linkMap = (0 to k).flatMap(nbTrue =>
        (0 to ds.size - k)
          .filter( _ + nbTrue < ds.size)
          .map(nbFalse =>
            (nbTrue, nbFalse) -> (
              ds(nbTrue + nbFalse),
              if (nbTrue == k - 1 && nbFalse == ds.size - k) oneIdx else if (nbTrue >= k) zeroIdx else (nbTrue + 1, nbFalse),
              if (nbTrue == k && nbFalse == ds.size - k - 1) oneIdx else if (nbFalse >= ds.size - k) zeroIdx else (nbTrue, nbFalse + 1)
            )
          )).toMap
  ...
  /**
    ...
    * @param replace the initial variable to replace
  ...
  /**
    ...
    * @param variable the variable labelling the BDD
    * @param high the high son
    * @param low the low son
    * @return the resulting BDD
    */
  ...
  /**
    * @return the mapping from BDD to labelled variable
    */
  def getVarMap: Map[MyBDD, Var]

  /**
    * Import a BDD in this factory from one coming from another factory
    * @param bdd the other factory BDD
    * @param bddVar the map from BDD node to variables
    * @tparam OtherBDD the type of the other BDD
    * @return the BDD imported in this factory
    */
  def importBDD[OtherBDD <: JavaBDD](bdd: OtherBDD, bddVar: Map[Var, OtherBDD]): MyBDD = {
  ...
  /**
    ...
    * @return the BDD imported in this factory
    */
  private def importRec[OtherBDD <: JavaBDD](bdd: OtherBDD, bddMap: Map[Int, Var]): MyBDD = {
    if (bdd.isOne)
      one()
    else if (bdd.isZero)
      zero()
    else
      mkNode(bddMap(bdd.`var`()), importRec(bdd.high(), bddMap), importRec(bdd.low(), bddMap))
  }
...
trait GenBDDFactory[Var] extends BaseBDDFactory[Var, JavaBDD] {
  ...
  object FactoryImplicits {
    ...
    /**
      ...
      * @param variable the variable labelled on the root node
      * @return the BDD "< v, 1, 0>"
      */
  ...
  /**
    * Try to find the BDD of the variable in the correspondence table
    * and return it if existing, otherwise generate a new BDD
    * @param variable the variable
    * @return a BDD node
    */
  ...
  def getPathCount(bdd:JavaBDD, weights:Map[Var,Int] = Map.empty): BigInt = {
    if(weights.isEmpty)
      ...
      def memoRedSatCount(x: JavaBDD): BigInt = _memo.getOrElseUpdate(x,
        if (x.isOne)
          1
        else if (x.isZero)
          0
          ...
          (for {v <- getVarOf(x)
                w <- weights.get(v)} yield
            memoRedSatCount(x.low()) + w * memoRedSatCount(x.high())).getOrElse (memoRedSatCount(x.low()) + memoRedSatCount(x.high()))
      )
  ...
  def getSatCount(bdd: JavaBDD, weights: Map[Var, Int] = Map.empty): BigInt = {
    ...
    def memoRedSatCount(x: JavaBDD, remainingDecisions:Int): BigInt = _memo.getOrElseUpdate((x,remainingDecisions),
      if (x.isOne)
        ...
        (for {v <- getVarOf(x)
              w <- weights.get(v)} yield
          memoRedSatCount(x.low(), remainingDecisions - 1) + w * memoRedSatCount(x.high(), remainingDecisions - 1)).getOrElse (memoRedSatCount(x.low(), remainingDecisions - 1) + memoRedSatCount(x.high(), remainingDecisions - 1))
    )
  ...
  def getVarOf(bdd: JavaBDD): Option[Var] =
    for {r <- varMap.find(_._2 == bdd.`var`())}
      yield r._1
}
