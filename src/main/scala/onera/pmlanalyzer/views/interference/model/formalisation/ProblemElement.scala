...
final case class AssertPB(l: Set[ALit], comp: Comparison, k: Int)
    extends Assert {
  def assert(s: Solver): Unit =
    s.assertPB(l.map(_.toLit(s)).toSeq.asJava, comp, k)
}
...
final case class MGraph(nodes: Set[MNode], edges: Set[MEdge])
  ...
  val toGraph: Solver => Graph = immutableHashMapMemo { s =>
    {
      val g = new Graph(s)

      val addNode: MNode => Int = immutableHashMapMemo { s =>
        g.addNode(s.id.name)
      }

      val addEdge: MEdge => Lit = immutableHashMapMemo { e =>
        g.addUndirectedEdge(addNode(e.from), addNode(e.to), e.id.name)
      }
...
final case class MEdgeLit(edge: MEdge, graph: MGraph) extends ALit {
  val toLit: Solver => Lit = immutableHashMapMemo { s =>
    graph.toGraph(s).getEdge(edge.id.name).l
  }
}
  ...
  def decodeModel(
    ...
    if (maxSize.exists(model.size > _))
      ...
      while (s.solve()) {
        ...
        s.assertOr(
          (positiveModel.map(variables).map(not) ++ negativeModel.map(
            variables
          )).asJava
        )
      }
