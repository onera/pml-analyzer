...
/**
  ...
  */
trait TableBasedInterferenceSpecification extends InterferenceSpecification
  ...
  /**
    ...
    */
  final lazy val services: Set[Service] = implicitly[Provided[Platform, Service]].apply(self)
  ...
  /**
    ...
    *  @return a set of services
    */
  final def transactionNotInterfereWith(t: PhysicalTransactionId): Set[Service] =
  ...
  /**
    ...
    */
  final def exclusiveWith(l: PhysicalTransactionId, r: PhysicalTransactionId): Boolean = {
    ...
    val differentAppUsed = Application.all
      ...
      .filter(s => {
        ...
        lUsed.contains(al) && rUsed.contains(ar) || lUsed.contains(ar) && rUsed.contains(al)
      })
    val appExclusive = differentAppUsed.nonEmpty && differentAppUsed.forall(s => {
      val (al, ar) = (s.head, s.last)
