...
trait InterferenceSpecification
  extends Transform.BasicInstances {
  self: Platform =>
  ...
  /**
    ...
    */
  final lazy val purifiedTransactions: Map[PhysicalTransactionId, PhysicalTransaction] =
  ...
  /**
    ...
    * @group transaction_relation
    */
  final lazy val purifiedTransactionsName: Map[PhysicalTransaction, PhysicalTransactionId] =
    purifiedTransactions.groupMapReduce(_._2)(_._1)((l, _) => l)

  /**
    * compute the considered scenarios depending on the configuration and the (optional) library, a scenario is either:
    * a named and used transaction (e.g. val t = Transaction(a read b); t.used)
    * a named and used scenario (e.g. val s = Scenario(t1, t2); s.used)
    * an anonymous copy (e.g. a copy r on s)
    * an anonymous transaction (e.g. a read b) not already involved in a copy, a named scenario or a named transaction
    * WARNING this will discard an anonymous transaction defined inside and outside a copy,
    * this issue does not occur if we keep the segregation Smart/NonSmart
    * @group scenario_relation
    * @return the set of scenarios
    */
  final lazy val purifiedScenarios: Map[PhysicalScenarioId, PhysicalScenario] = this match {
  ...
  /**
    ...
    * @return true is the transaction is discarded
    */
  def isTransparentTransaction(t: PhysicalTransactionId): Boolean
  ...
  /**
    ...
    */
  def transactionInterfereWith(t: PhysicalTransactionId): Set[Service]
  ...
  /**
    ...
    * @return a set of services
    */
  def transactionNotInterfereWith(t: PhysicalTransactionId): Set[Service]

  /**
    * Check whether two hardware cannot work simultaneously
    * @group interfere_predicate
    * @param l the left hardware
    * @param r the right hardware
    * @return true if they cannot work simultaneously
    */
  ...
  /**
    ...
    * @param l the left transaction
    * @param r the right transaction
    * @return true if they cannot occur simultaneously
    */
  final def finalExclusive(l: PhysicalTransactionId, r: PhysicalTransactionId): Boolean = {
    antiReflexive(l, r) && symmetric[PhysicalTransactionId](exclusiveWith)(l, r)
  }

  /**
    * Check whether two scenarios will not occur simultaneously
    * @group exclusive_predicate
    * @param l the left scenarios
    * @param r the right scenarios
    * @return true if they cannot occur simultaneously
    */
  final def finalExclusive(l: PhysicalScenarioId, r: PhysicalScenarioId): Boolean =
    antiReflexive(l, r) &&
      symmetric((le: PhysicalScenarioId, re: PhysicalScenarioId) =>
        purifiedScenarios(le).exists(t =>
          purifiedScenarios(re).exists(tp =>
            finalExclusive(t, tp))))(l, r)

  /**
    * Provide the map encoding of finalInterfereWith
    * @group exclusive_predicate
    * @param s the set of scenario
    * @return the map encoding
    */
  final def finalExclusive(s: Set[PhysicalScenarioId]): Map[PhysicalScenarioId, Set[PhysicalScenarioId]] =
  ...
  /**
    ...
    * @param s the set of [[pml.model.service.Service]]
    * @return the equivalence classes
    */
  final def serviceEquivalenceClasses(s: Set[Service]): Set[Set[Service]] =
    equivalenceClasses[Service]((le, re) => reflexive(le, re) || symmetric(areEquivalent)(le, re))(s)

  /**
    * Provide the equivalence classes over s with [[views.interference.operators.Equivalent.Ops]] relation
    * @group equivalence_predicate
    * @param s the set of [[views.interference.model.specification.InterferenceSpecification.PhysicalTransactionId]]
    * @return the equivalence classes
    */
  final def equivalenceTransactionClasses(s: Set[PhysicalTransactionId]): Set[Set[PhysicalTransactionId]] = {
    val serviceClasses = serviceEquivalenceClasses(s.flatMap(transactionsByName.get).flatMap(_.toSet))
    val areEquivalentTr = (l: PhysicalTransactionId, r: PhysicalTransactionId) => {
      transactionsByName.contains(l) &&
        ...
        transactionsByName(l)
          ...
          .forall(p => serviceClasses.exists(c => c.contains(p._1) && c.contains(p._2)))
    }

    equivalenceClasses[PhysicalTransactionId]((le, re) => reflexive(le, re) || symmetric(areEquivalentTr)(le, re))(s)
  }

  /**
    * Check if it exists at least one common service used by two set of scenarios
    * @group utilFun
    * @param l the left set of scenarios
    * @param r the right set of scenarios
    * @return true whether one channel exists
    */
  ...
  /**
    ...
    * @group utilFun
    * @param s the set of gathered scenarios
    * @return the map encoding
    */
  ...
  /**
    ...
    * @group interfere_predicate
    * @param l the left service
    * @param r the right service
    ...
    antiReflexive(l, r) && (
      symmetric[Service](interfereWith)(l, r) ||
        (l.hardwareOwner.nonEmpty &&
          ...
          l.hardwareOwner.exists(ol => r.hardwareOwner.exists(or => finalInterfereWith(ol, or))) //service owner are exclusive
          )
      )
  }
  ...
  /**
    ...
    * @param l the left service
    * @param r the right service
    * @return true if the services are equivalent
    */
  ...
  /**
    ...
    * @return true if they cannot occur simultaneously
    */
  protected def exclusiveWith(l: PhysicalTransactionId, r: PhysicalTransactionId): Boolean
  ...
  protected def relationToMap[T](all: => Set[T], r: (T, T) => Boolean): Map[T, Set[T]] =
  ...
  private def addToClosure[T](relation: (T, T) => Boolean)(s: T, closures: Set[Set[T]]): Set[Set[T]] =
  ...
  private def equivalenceClasses[T](relation: (T, T) => Boolean)(in: Set[T]): Set[Set[T]] = {
    val closure = addToClosure[T](relation) _
    in.foldLeft(Set.empty[Set[T]])((acc, s) => closure(s, acc))
  }

  @deprecated("Poor performance implementation, consider using MONOSAT problem to identify channel")
  final def channel(t: PhysicalTransactionId, in: Set[PhysicalTransactionId]): Set[Service] =
    (for {ts <- purifiedTransactions.get(t)
          ss = ts.toSet} yield {
      in
  ...
  @deprecated("Poor performance implementation, consider using recursive transaction equivalence classes splitting perhaps implemented within MONOSAT")
  final def equivalentTransactionSets(s: Set[PhysicalTransactionId], in: Set[PhysicalTransactionId]): Set[Set[PhysicalTransactionId]] = {
    val transactionsClasses = equivalenceTransactionClasses(in)
      .flatMap(c => c.intersect(s).map(_ -> c))
      .toMap
    s.foldLeft((Set.empty[PhysicalTransactionId], Set.empty[Set[PhysicalTransactionId]]))((acc, t) => {
      val (current, result) = acc
      val iniChannel = channel(t, current)
      if (current.isEmpty) {
        (current + t, (transactionsClasses(t) - t).map(Set(_)))
      } else {
        val r = (current + t, result.flatMap(st =>
          (transactionsClasses(t) -- st - t)
            .filter(tp => channel(tp, st) == iniChannel && st.forall(!finalExclusive(_, tp)))
            .map(st + _)))
        r
      }
    })._2
...
object InterferenceSpecification {
  type Path[A] = List[A]
  type PhysicalTransaction = Path[Service]
  type PhysicalScenario = Set[PhysicalTransactionId]
  ...
  trait Default extends InterferenceSpecification {
    ...
    def isTransparentTransaction(t: PhysicalTransactionId): Boolean = false

    def transactionInterfereWith(t: PhysicalTransactionId): Set[Service] = Set.empty

    def transactionNotInterfereWith(t: PhysicalTransactionId): Set[Service] = Set.empty
