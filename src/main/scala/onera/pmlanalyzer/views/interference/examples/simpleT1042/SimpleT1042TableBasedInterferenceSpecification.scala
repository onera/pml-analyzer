...
import onera.pmlanalyzer.pml.examples.simpleT1042.{
...
trait SimpleT1042PhysicalTableBasedInterferenceSpecification
  ...
  for {
    ...
    r <- services
    if l != r
    if (l.hardwareOwnerIs(dma) && r.hardwareOwnerIs(
      config_bus
    )) || (l.hardwareOwner == r.hardwareOwner && !l.hardwareOwnerIs(bus))
  } yield {
    l interfereWith r
  }
...
trait SimpleT1042ApplicativeTableBasedInterferenceSpecification
    extends ApplicativeTableBasedInterferenceSpecification {
  self: SimpleT1042Platform
    with SimpleT1042TransactionLibrary
    with SimpleT1042LibraryConfiguration
