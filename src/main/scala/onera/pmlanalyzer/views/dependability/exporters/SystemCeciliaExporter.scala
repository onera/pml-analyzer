...
trait SystemCeciliaExporter {
  ...
  implicit def systemIsExportable[T <: System]: Aux[T, SystemModel] =
    new CeciliaExporter[T] {
      type R = SystemModel

      def toCecilia(x: T): SystemModel = {
        val targets = x.context.portOwner.values.collect {
          case t: InputDepTarget[_] => t
        }.toSet
        val epsilons = targets.map { t =>
          DeterministicEventModel(
            Symbol(s"$t.${t.fMAutomaton}.${t.fMAutomaton.epsilon}")
          )
        }
        val fires = targets.toList.sortBy(_.id.name).map { t => s"$t.fire" }
        val directions =
          targets.toList.sortBy(_.id.name).map { t => s"$t.direction" }
        val scheduler = WorstSchedulerTopHelper(epsilons.size)
        val schedulerModel =
          SubComponent(Symbol("worstScheduler"), scheduler.model)
        val topLevelPort = (v: Variable[_]) =>
          !x.context.componentOwner.isDefinedAt(x.context.portOwner(v.id))
        val topLevelConnections = x.context.links.collect {
          case (i, s) if topLevelPort(i) =>
            if (s.size != 1)
              s.zipWithIndex.map(p =>
                s"${variablePathName(x, i)}${p._2} = ${variablePathName(x, p._1)};"
              )
            else
              Set(
                s"${variablePathName(x, i)} = ${variablePathName(x, s.head)};"
              )
        }.flatten
        SystemModel(
          x.name,
          ...
          s"""assert
           ...
           |${fires
              .zip(scheduler.fireOrders)
              .map(p => s"${p._1} = $schedulerModel.${p._2};")
              .mkString("\n")}
           |${scheduler.sonDirection
              .zip(directions)
              .map(p => s"$schedulerModel.${p._1} = ${p._2};")
              .mkString("\n")}
           |""".stripMargin
        )
      }
