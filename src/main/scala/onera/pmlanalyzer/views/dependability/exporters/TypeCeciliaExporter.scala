...
trait TypeCeciliaExporter {
  def typeModel[T: IsFinite]: EnumeratedType = {
    EnumeratedType(
      nameOf[T],
      allWithNone.map(fm => fm.name).toList,
      PhylogFolder.phylogFMTypeFolder
    )
  }
  ...
  def typeModel[K: IsFinite, V: IsFinite]: RecordType = {
    ...
    RecordType(
      Symbol(s"Map${nameOf[K].name}To${nameOf[V].name}"),
      PhylogFolder.phylogCustomTypeFolder,
      allOf[K].map(t => Flow(t.name, tyypeMode, In)).toList
    )
  }

  def boolMapTypeModel[K: IsFinite]: RecordType = {
    RecordType(
      Symbol(s"Map${nameOf[K].name}ToBool"),
      PhylogFolder.phylogCustomTypeFolder,
      allOf[K].map(t => Flow(t.name, CeciliaBoolean, In)).toList
    )
  }
}
