...
trait SoftwareCeciliaExporter {
  ...
  implicit def dependencySpecifiedSoftwareIsExportable[FM: IsFinite : IsCriticityOrdering]: Aux[Application[FM], EquipmentModel] = new CeciliaExporter[Application[FM]] {
    ...
    def toCecilia(x: Application[FM]): EquipmentModel = {
      ...
      val stateExpr = ExprCeciliaExporter.toCecilia(x.softwareState(x.coreState,x.loadI))
      val storeExpr = ExprCeciliaExporter.toCecilia(x.stores(x.coreState,x.loadI))
      val subComponents =
      ...
      EquipmentModel(
        ...
        s"""assert
           ...
           |${allOf[TargetId].map(k => s"$requestO^$k = ${(for(s <- storeExpr.result.get(k)) yield s) getOrElse noneOf[FM].name.name};").sorted.mkString("\n")}
           |
  ...
  implicit def descriptorIsExportable[FM: IsFinite : IsCriticityOrdering]: Aux[Descriptor[FM], EquipmentModel] = new CeciliaExporter[Descriptor[FM]] {
    type R = EquipmentModel
    def toCecilia(x: Descriptor[FM]): EquipmentModel = {
      ...
      val storeDependency = x.transferts.flatMap(c => c.targetWritten.map(store => store -> c.targetNeeded)).filter(_._2.nonEmpty).toMap
      val worst = storeDependency.transform((_,v) => mkWorstSub(v.map(t => s"$loadI^$t")))
