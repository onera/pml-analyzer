...
trait BasicOperationCeciliaExporter {
  ...
  def pathName(x: DepSystem, c: Component): List[String] = x.context.componentOwner.get(c) match {
  ...
  def mkVariableName(v: VariableId, tyype: CeciliaType, orientation: Orientation, arity: Int): List[Flow] = arity match {
    case 0 => Nil
    case 1 => Flow(v.name, tyype, orientation) :: Nil
    case n => (0 until n).map(i => Flow(Symbol(s"$v$i"), tyype, orientation)).toList
  }
  ...
  def configurableCstModel[T: IsFinite : IsCriticityOrdering]: ComponentModel = {
    ...
    ComponentModel(Symbol("constant"),
  ...
  def configurableCstModel[K: IsFinite, V: IsFinite : IsCriticityOrdering]: ComponentModel = {
    val tyype = typeModel[K, V]
    val output = Flow(Symbol("o"), tyype, Out)
    val states = tyype.fields.map(field => State(field.name, field.tyype, min[V].name.name))
    ComponentModel(Symbol("constant"),
      SubFamilyFolder(tyype.name, PhylogFolder.fmOperatorsFamilyFolder),
      GenericImage.sourceBlueCircle,
      Nil,
  ...
  def switchModel[T: IsFinite : IsCriticityOrdering]: ComponentModel = {
    ...
    ComponentModel(
      ...
      Flow(Symbol("select1"), CeciliaBoolean, In) :: Flow(Symbol("i1"), tyype, In) :: Flow(Symbol("i2"), tyype, In) :: Flow(Symbol("o"), tyype, Out) :: Nil,
      Nil,
      Nil,
      s"""assert
  ...
  def preModel[T: IsFinite : IsCriticityOrdering]: ComponentModel = {
    ...
    ComponentModel(
      ...
      Flow(Symbol("currentValue"), tyype, In) :: Flow(Symbol("preValue"), tyype, Out) :: Nil,
  ...
  private val _bestMemo = collection.mutable.HashMap.empty[Set[String],(String,Seq[String],SubComponent)]

  def mkBestSub[T: IsFinite : IsCriticityOrdering](l:Set[String]): (String,Seq[String],SubComponent) = _bestMemo.getOrElseUpdate(l,{
    val model = BestModelHelper[T](l.size)
    val uuid = _bestMemo.keys.count(p => p.size == l.size)
    val subName = s"${model.name.name}$uuid"
    val worstAssertions = model.inputs.map{f =>  s"$subName.$f"} zip l map (p => s"${p._1} = ${p._2};")
  ...
  def minOperator[T: IsFinite : IsCriticityOrdering](size: Int): OperatorModel = {
    val tyype = typeModel[T]
    val inputs = (1 to size).map(i => Flow(Symbol(s"i$i"), tyype, In)).toList
    val output = Flow(Symbol(s"min$size${nameOf[T].name}"), tyype, Out)
    OperatorModel(PhylogFolder.genericOperatorFolder, inputs, output,
      s"""assert
         |$output = case {
         |${
        allOf[T].toList.sorted.map(fm => inputs.map(f => s"$f = ${fm.name.name}").mkString(" or ") + s": ${fm.name.name},").mkString("", "\n", s"\nelse ${noneOf[T].name.name}")
      }
         |};
       """.stripMargin)
  }

  private val _worstMemo = collection.mutable.HashMap.empty[Set[String],(String,Seq[String],SubComponent)]

  def mkWorstSub[T: IsFinite : IsCriticityOrdering](l:Set[String]): (String,Seq[String],SubComponent) = _worstMemo.getOrElseUpdate(l,
    {
  ...
  private final case class WorstModelHelper[T: IsFinite : IsCriticityOrdering](size: Int) {
    ...
    val model: ComponentModel = ComponentModel(
      ...
      s"""assert
         |$result = case {
         |${allOf[T].toList.sorted.reverse.map(fm => inputs.map(f => s"${f.name.name} = ${fm.name.name}").mkString(" or ") + s": ${fm.name.name},").mkString("", "\n", s"\nelse ${noneOf[T]}")}
         |};
       """.stripMargin)
  }

  def maxOperator[T: IsFinite : IsCriticityOrdering](size: Int): OperatorModel = {
    val tyype = typeModel[T]
    val inputs = (1 to size).map(i => Flow(Symbol(s"i$i"), tyype, In)).toList
  ...
  def mkContainerShadowSub[T: IsFinite : IsShadowOrdering](name: Symbol, newMode:String, containerMode:String): (String,Seq[String],SubComponent) = {
    val model = new ContainerShadowHelper[T]()
    val worstAssertions = s"${name.name}.${model.newMode} = $newMode;" :: s"${name.name}.${model.containerMode} = $containerMode;" :: Nil
    (s"${name.name}.${model.output}",worstAssertions,SubComponent(name,model.model))
  }
  ...
  private class ContainerShadowHelper[T: IsFinite : IsShadowOrdering] {
    ...
    private val impacted = allOf[T].map(container => container -> allOf[T].filter(current => current.containerShadow(container) == container)).filter(_._2.nonEmpty)
    val model: ComponentModel = ComponentModel(
      ...
      s"""assert
         |$output = case {
         |${impacted.map(p => "(" + p._2.map(current => s"$newMode = $current").mkString("(", " or ", ")") + s" and $containerMode = ${p._1.name.name})").mkString(" or ")} : $containerMode,
         |else $newMode
         |};
       """.stripMargin)
  }
  ...
  def containerShadowOperator[T: IsFinite : IsShadowOrdering]: OperatorModel = {
    ...
    val impacted = allOf[T].map(container => container -> allOf[T].filter(current => current.containerShadow(container) == container)).filter(_._2.nonEmpty)
    OperatorModel(
      PhylogFolder.genericOperatorFolder, newMode :: containerMode :: Nil, output,
      s"""assert
         |$output = case {
         |${impacted.map(p => "(" + p._2.map(current => s"$newMode = $current").mkString("(", " or ", ")") + s" and $containerMode = ${p._1.name.name})").mkString(" or ")} : $containerMode,
         |else $newMode
  ...
  def inputShadowOperator[T: IsFinite : IsShadowOrdering]: OperatorModel = {
    ...
    val impacted = allOf[T].map(container => container -> allOf[T].filter(current => current.inputShadow(container) == container)).filter(_._2.nonEmpty)
    OperatorModel(
      PhylogFolder.genericOperatorFolder, newMode :: containerMode :: Nil, output,
      s"""assert
         |$output = case {
  ...
  final case class WorstSchedulerTopHelper(size:Int) {
    ...
    private val fireOrdersAssertions = fireOrders.zip(sonDirection).map( p =>
      s"""${p._1} = case {
         ...
         |${sonDirection.filterNot(_ == p._2).map(other => s"$other = ${Direction.Degradation}").mkString(" or ")} : ${Fire.Wait},
         |else ${Fire.No}
         |};""".stripMargin
    )
    val model: ComponentModel = ComponentModel(
      Symbol(s"worstScheduler$size"),
      SubFamilyFolder(typeModel[Fire.Value].name, PhylogFolder.phylogComponentFolder),
      GenericImage.maxBlue,
      GenericImage.maxGreen :: GenericImage.maxRed :: Nil,
  ...
  def authorizeOperator[FM: IsFinite]: OperatorModel = {
    ...
    OperatorModel(
      PhylogFolder.genericOperatorFolder, initial :: reject :: Nil, output,
      s"""assert
