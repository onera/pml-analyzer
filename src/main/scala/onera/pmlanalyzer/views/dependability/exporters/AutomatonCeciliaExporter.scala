...
trait AutomatonCeciliaExporter {
  ...
  implicit def simpleFMAutomatonIsExportable[T: IsCriticityOrdering : IsFinite]: Aux[SimpleFMAutomaton[T], ComponentModel] = new CeciliaExporter[SimpleFMAutomaton[T]] {
  ...
  implicit def inputFMAutomatonIsExportable[T: IsCriticityOrdering : IsFinite : IsShadowOrdering]: Aux[InputFMAutomaton[T], ComponentModel] = new CeciliaExporter[InputFMAutomaton[T]] {
    type R = ComponentModel
    ...
    def toCecilia(x: InputFMAutomaton[T]): ComponentModel = {
      ...
      val nextStateAssertion = allOf[T].flatMap(to => allOf[T].collect({
