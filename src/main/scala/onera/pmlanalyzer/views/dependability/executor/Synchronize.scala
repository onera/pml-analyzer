...
object Synchronize {
  ...
  private val synchronizations : mutable.Set[SynchroEvent] = mutable.Set.empty[SynchroEvent]
  ...
  def addSynchro(s:Set[Event]) : SynchroEvent = {
    ...
    (for (sync <- synchronizations.find(s2 => s2.synchronizedEvents.intersect(s).nonEmpty)) yield {
      val newSync = sync.copy(synchronizedEvents = sync.synchronizedEvents.union(s))
  ...
  def isFireable(s:SynchroEvent) : Boolean =
    s.synchronizedEvents.foldLeft(true)((acc,e) => e match {
      case concrete:ConcreteEvent => acc && concrete.owner.fireable(concrete).nonEmpty
