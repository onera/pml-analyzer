...
private[pmlanalyzer] object IsFinite {
  implicit def tupleIsFinite[T, U](implicit
      evT: IsFinite[T],
      evU: IsFinite[U]
  ): IsFinite[(T, U)] = new IsFinite[(T, U)] {
    override def typeName: Symbol = Symbol(
      s"Pair${evT.typeName.name}${evU.typeName.name}"
    )
    def allWithNone: Seq[(T, U)] =
      evT.all.flatMap(t => evU.all.map(u => (t, u))) :+ none
    def name(x: (T, U)): Symbol = Symbol(
      s"${evT.name(x._1).name}_${evU.name(x._2).name}"
    )
