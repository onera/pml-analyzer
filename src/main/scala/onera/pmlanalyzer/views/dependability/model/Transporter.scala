...
abstract class BasicTransporter [FM: IsCriticityOrdering : IsFinite : IsShadowOrdering](implicit owner: Owner) extends Transporter[FM] {
  val loadO: OutputPort[Request[FM]]
  val storeI: InputPort[List[Request[FM]]]
}

class SimpleTransporter [FM: IsCriticityOrdering : IsFinite : IsShadowOrdering] private(val id : TransporterId,val reject: ((InitiatorId,TargetId)) => Boolean)(implicit owner: Owner) extends BasicTransporter[FM]{
  val storeI: InputPort[List[Request[FM]]] = InputPort[List[Request[FM]]](Symbol("storeI"))
  val storeO : OutputPort[Request[FM]] =
    OutputPort(VariableId(Symbol("storeO")), () =>
...
object SimpleTransporter {
  def apply[T: IsCriticityOrdering : IsFinite : IsShadowOrdering](id: TransporterId, reject: ((InitiatorId, TargetId)) => Boolean)(implicit context: Builder[SubComponent] with Owner): SimpleTransporter[T] = {
...
class Virtualizer [FM: IsCriticityOrdering : IsFinite : IsShadowOrdering] private(val id : TransporterId,val reject: ((InitiatorId,TargetId)) => Boolean)(implicit owner: Owner) extends BasicTransporter[FM]{
  val storeI: InputPort[List[Request[FM]]] = InputPort[List[Request[FM]]](Symbol("storeI"))
  val storeO : OutputPort[Request[FM]] =
    OutputPort(VariableId(Symbol("storeO")), () =>
      for(reqs <- storeI.eval(); s <- fMAutomaton.o.eval()) yield {
...
object Virtualizer {
  def apply[T: IsCriticityOrdering : IsFinite : IsShadowOrdering](id: TransporterId, reject: ((InitiatorId, TargetId)) => Boolean)(implicit context: Builder[SubComponent] with Owner): Virtualizer[T] = {
...
class Initiator [FM: IsCriticityOrdering : IsFinite : IsShadowOrdering] private(val id : InitiatorId)(implicit owner: Owner) extends Transporter[FM]{
  val storeI: InputPort[List[TargetStatus[FM]]] = InputPort[List[TargetStatus[FM]]](Symbol("storeI"))
  val storeO : OutputPort[Request[FM]] =
    OutputPort(VariableId(Symbol("storeO")), () =>
      for(reqs <- storeI.eval(); s <- fMAutomaton.o.eval()) yield {
        reqs.map(r => {
...
object Initiator {
  def apply[T: IsCriticityOrdering : IsFinite : IsShadowOrdering](id: InitiatorId)(implicit context: Builder[SubComponent] with Owner): Initiator[T] = {
