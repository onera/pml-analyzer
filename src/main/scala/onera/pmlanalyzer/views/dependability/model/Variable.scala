...
final case class DMap[T](m: Map[TargetId, Expr[T]])
...
final case class Of[T](m: Variable[Map[TargetId, T]], id: TargetId)
    extends Expr[T] {
...
final case class OutputPort[T](id: VariableId, evalFun: () => Option[T])
    extends Variable[T] {
  def eval(): Option[T] = for (x <- evalFun()) yield x
  def |+|(that: OutputPort[T]): List[OutputPort[T]] = this :: that :: Nil
}

final case class LocalVariable[T](id: VariableId, evalFun: () => Option[T])
    extends Variable[T] {
  def eval(): Option[T] = for (x <- evalFun()) yield x
}
