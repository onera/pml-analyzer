...
trait Target[FM] extends Component {
  ...
  val loadO: OutputPort[Request[FM]] =
    OutputPort(
      ...
      () =>
        for (o <- fMAutomaton.o.eval())
          yield Request(allOf[InitiatorId].map(sId => (sId, id) -> o): _*)
    )
}
...
class InputDepTarget[
  ...
  val fMAutomaton: InputFMAutomaton[FM] =
    InputFMAutomaton[FM](AutomatonId(Symbol("fmAutomaton")), min[FM])
  fMAutomaton.in := {
    for (requests <- storeI.eval()) yield {
      val stores = requests.flatMap { r =>
        r.collect { case (k, v) if k._2 == id => v }
      }
      if (stores.isEmpty)
        noneOf[FM]
      else
