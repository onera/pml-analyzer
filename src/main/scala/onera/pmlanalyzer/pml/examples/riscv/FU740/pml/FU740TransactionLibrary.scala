...
trait FU740TransactionLibrary extends TransactionLibrary {
  ...
  /* FIXME Issue number included */
  def Issue47(name: String, from: Scenario): Scenario =
    Scenario(UserScenarioId(Symbol(name)), from.iniTgt, from.sw)
  ...
  private val t0_rd_mem: Transaction = Transaction(app0 read d0.Mem)
...
//  val t0_5: Scenario = Scenario(t0_wr_mem, t0_l2_refill)
  ...
  private val t1_rd_l2: Transaction = Transaction(app1 read d1.L2Cache)
  private val t1_wr_l2: Transaction = Transaction(app1 write d1.L2Cache)
  private val t1_rd_mem: Transaction = Transaction(app1 read d1.Mem)
  private val t1_wr_mem: Transaction = Transaction(app1 write d1.Mem)
  private val t1_l1_refill: Transaction = Transaction(app1 write d1.L1DCache)
  private val t1_l2_refill: Transaction = Transaction(app1 write d1.L2Cache)

  val t1_0: Transaction = Transaction(app1 read d1.L1DCache)
  val t1_1: Transaction = Transaction(app1 write d1.L1DCache)
...
//  val t1_4: Transaction = Transaction(app1 read di)
//  val t1_5: Transaction = Transaction(app1 write di)

  private val t2_wr_l2: Transaction = Transaction(app2 write d2.L2Cache)
  private val t2_rd_l2: Transaction = Transaction(app2 read d2.L2Cache)
  private val t2_rd_mem: Transaction = Transaction(app2 read d2.Mem)
  private val t2_wr_mem: Transaction = Transaction(app2 write d2.Mem)
  private val t2_l1_refill: Transaction = Transaction(app2 write d2.L1DCache)
  private val t2_l2_refill: Transaction = Transaction(app2 write d2.L2Cache)

  val t2_0: Transaction = Transaction(app2 read d2.L1DCache)
  val t2_1: Transaction = Transaction(app2 write d2.L1DCache)
...
//  val t2_5: Scenario = Scenario(t2_wr_mem, t2_l2_refill, t2_l1_refill)
  ...
  private val t3_rd_l2: Transaction = Transaction(app3 read d3.L2Cache)
  private val t3_wr_l2: Transaction = Transaction(app3 write d3.L2Cache)
  private val t3_rd_mem: Transaction = Transaction(app3 read d3.Mem)
  private val t3_wr_mem: Transaction = Transaction(app3 write d3.Mem)
  private val t3_l1_refill: Transaction = Transaction(app3 write d3.L1DCache)
  private val t3_l2_refill: Transaction = Transaction(app3 write d3.L2Cache)

  val t3_0: Transaction = Transaction(app3 read d3.L1DCache)
  val t3_1: Transaction = Transaction(app3 write d3.L1DCache)
...
//  val t3_5: Scenario = Scenario(t3_wr_mem, t3_l2_refill, t3_l1_refill)
  ...
  private val t4_rd_l2: Transaction = Transaction(app4 read d4.L2Cache)
  private val t4_wr_l2: Transaction = Transaction(app4 write d4.L2Cache)
  private val t4_rd_mem: Transaction = Transaction(app4 read d4.Mem)
  private val t4_wr_mem: Transaction = Transaction(app4 write d4.Mem)
  private val t4_l1_refill: Transaction = Transaction(app4 write d4.L1DCache)
  private val t4_l2_refill: Transaction = Transaction(app4 write d4.L2Cache)

  val t4_0: Transaction = Transaction(app4 read d4.L1DCache)
  val t4_1: Transaction = Transaction(app4 write d4.L1DCache)
