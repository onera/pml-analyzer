...
private[pmlanalyzer] object Linked {
  ...
  /** If an element l of type L is linked to other elements of type R then the
    * operator can be used
    *
    * To access to the element of type R linked to l {{{l.linked[R]}}} To access
    * to the element of type R pointing to l {{{l.inverse[R]}}}
    * @note
    *   Linked operators is an advanced feature and should not be necessary for
    *   basic models
    */
  trait Ops {
  ...
  /** A linked implementation over loads can be derived from an endomorphism
    * over a services
    * @return
    *   the implementation of the linked
    */
  given [T <: Load | Store: Typeable](using
