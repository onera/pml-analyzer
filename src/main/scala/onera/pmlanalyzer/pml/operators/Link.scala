...
/** Base trait for link operation
  ...
  */
trait Link[L, R] {
  def link(a: L, b: R)(using _line: Line, _file: File): Unit

  def unlink(a: L, b: R)(using _line: Line, _file: File): Unit
}
...
object Link {
  ...
  /** If an element x of type T is linkable then the operator can be used as
    * follows
    *
    * to link an element l to an element r {{{l link r}}} to unlink an element l
    * with an element r {{{l unlink r}}}
    *
    * @note
    *   currently any [[pml.model.hardware.Hardware]] or
    *   [[pml.model.service.Service]] are linkable with soundness restriction.
    *   Additionally, the link can be made only within a platform container
    * @see
  ...
  trait Ops {
    ...
    extension [L](self: L) {
      ...
      /** PML keyword to link two objects
        ...
        */
      def link[R](b: R)(using linkable: Link[L, R], _line: Line, _file: File): Unit =
      ...
      /** PML keyword to unlink two objects
        ...
        *   the type of the other object
        */
      def unlink[R](b: R)(using linkable: Link[L, R], _line: Line, _file: File): Unit =
  ...
  /** A linking implementation between two types can be derived from an
    * endomorphism over a supertype
    *
    * @return
  ...
  given [LS, RS](using
    ...
    def link(a: LS, b: RS)(using _line: Line, _file: File): Unit = l.add(canLink(a), canLink(b))

    def unlink(a: LS, b: RS)(using _line: Line, _file: File): Unit = l.remove(canLink(a), canLink(b))
  }

  /** A linking implementation can be provided for all physical component
    *
    * @return
    *   the implementation of the link for two physical components
    */
  given [LH, RH](using
      canLink: HardwareLink[LH, RH],
    ...
    def link(a: LH, b: RH)(using _line: Line, _file: File): Unit = {
    ...
    def unlink(a: LH, b: RH)(using _line: Line, _file: File): Unit = {
