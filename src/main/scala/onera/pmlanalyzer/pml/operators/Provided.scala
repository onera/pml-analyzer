...
private[pmlanalyzer] object Provided {
  ...
  /** If an element l of type L can provide element of type R then the operator
    * can be used
    *
    ...
    * {{{l.hardwareOwner}}} To check if an hardware r is providing an element l
    * {{{l.hardwareOwnerIs(r)}}}
    *
    * @note
  ...
  trait Ops {
    ...
    extension [L](self: L) {
      ...
      def targets(using ev: Provided[L, Hardware]): Set[Target] =
        ev(self).collect({ case x: Target => x })
    ...
    extension [R](self: R) {
      ...
      def hardwareOwnerIs(that: Hardware)(using
          ev: Provided[Hardware, R]
      ): Boolean = hardwareOwner.contains(that)
      ...
      def transporterOwner(using
          ev: Provided[Transporter, R]
      ): Set[Transporter] = ev.owner(self)
    }
  ...
  /** ------------------------------------------------------------------------------------------------------------------
    ...
    * ---------------------------------------------------------------------------------------------------------------
    */
  /** An implementation of the provide operator between two object is derivable
    * from a relation
    *
    * @return
  ...
  /** An implementation of the services provided by an hardware component is
    * derivable from the services provided by a physical component
    *
    * @return
    *   the implementation of the provided
    */
  given [T <: Hardware: Typeable, S <: Service: Typeable](using
      ev: Provided[Hardware, Service]
  ): Provided[T, S] with {
    def apply(a: T): Set[S] = ev(a) collect { case s: S =>
      s
    }

    def owner(b: S): Set[T] = ev.owner(b) collect { case s: T =>
      s
    }
  }
  ...
  /** An implementation of the services provided by a target on which a data is
    * allocated is derivable from the services provided by a target and the name
    * if the platform
    *
    * @return
    *   the implementation of the provided
