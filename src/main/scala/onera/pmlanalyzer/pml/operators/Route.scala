...
object Route {
  ...
  /**
    ...
    * @note the constraint is fruitful iff a service of the router is linked to a service of next
    * @see route usage can be found in [[pml.examples.simpleKeystone.SimpleRoutingConfiguration]]
    */
  trait Ops {
    ...
    extension(self:Initiator){
      /**
        ...
        * @return the partial routing constraint where the initiator target couple is specified
        */
      ...
      /**
        ...
        * @param target a set of targets
        * @return the partial routing constraint where the initiator target couples are specified
        */
      ...
      /**
        ...
        * @param router an hardware
        * @return a partial routing constraint where the initiator, the targets and the router are specified
        */
      def useLink(router: Hardware)(implicit owner: Owner): SimpleRouterIdentifyNext =
      ...
      /**
        ...
        * @return a partial routing constraint where the initiator, the targets and the router are specified
        */
      def cannotUseLink(router: Hardware)(implicit owner: Owner): SimpleRouterIdentifyNext =
    ...
    case class SimpleRouteIdentifyRouter(a: Initiator, targets: Iterable[Target]) {
      ...
      def blockedBy(router: Hardware)(implicit
                                      ...
                                      r:RoutingRelation[(Initiator, Service, Service), Service]): Unit = {
        for {t <- targets
             tL <- t.loads
             rL <- router.loads
             } yield {
    ...
    case class SimpleRouterIdentifyNext(a: Initiator, targets: Iterable[Target], router: Hardware, forbid:Boolean) {
      ...
      /**
        ...
        * @param next the component linked to [[router]]
        * @param p the proof that hardware provide services
        * @param l the proof that services can be linked
      ...
      def to(next: Hardware)(implicit
                             ...
                             l: Linked[Service, Service],
                             r:RoutingRelation[(Initiator, Service, Service), Service]): Unit = {
        if (!next.services.exists(s => router.services.exists(s2 => l(s2).contains(s))))
