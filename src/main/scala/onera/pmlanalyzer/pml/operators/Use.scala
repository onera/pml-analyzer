...
object Use {
  ...
  trait Ops {
    ...
    extension [L <: Application | Initiator](self: L) {
      ...
      private def use[B](b: B)(using ev: Use[L, B], line: Line, file: File): (L, B) = {
        ev(self, b)
        self -> b
      }

      private def use[B](b: Set[B])(using ev: Use[L, B], line: Line, file: File): Set[(L, B)] =
        b.map(x => use(x))
      ...
      def read[B](
          b: B
                 )(using p: Provided[B, Load], ev: Use[L, Load], line: Line, file: File): Set[(L, Load)] = use(
        b.loads
      )
      ...
      /** The PML keyword specify that self uses some load services
        ...
        *   the link
        */
      def read(b: Set[Service])(using ev: Use[L, Load], line: Line, file: File): Set[(L, Load)] = use(
      ...
      def write[B](
          b: B
                  )(using p: Provided[B, Store], ev: Use[L, Store], line: Line, file: File): Set[(L, Store)] = use(
        b.stores
      )
      ...
      /** The PML keyword specify that self uses some store services
        ...
        *   the link
        */
      def write(b: Set[Service])(using ev: Use[L, Store], line: Line, file: File): Set[(L, Store)] =
    ...
    extension [L <: Data](self: L) {
      ...
      /** The PML keyword to allocate self on an target
        ...
        *   the link
        */
      def hostedBy(b: Target)(using ev: Use[Data, Target], line: Line, file: File): (Data, Target) = {
        ev(self, b)
        self -> b
      }
    }
