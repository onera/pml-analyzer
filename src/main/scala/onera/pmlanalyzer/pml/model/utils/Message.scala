...
object Message {
  ...
  inline def impossibleRouteWarning(
      t: Service,
      from: Option[Application]
  ): String =
    s"""[WARNING] The target service $t cannot be reached ${
        if (from.isDefined) s"from ${from.getOrElse("unknown application")}"
        else ""
      }"""
  ...
  inline def uselessRoutingConstraintWarning(
      from: Hardware,
      to: Hardware
  ): String =
  ...
  inline def successfulITFDifferenceExportInfo(
      size: Any,
      x: Any,
      y: Any,
      file: Any
