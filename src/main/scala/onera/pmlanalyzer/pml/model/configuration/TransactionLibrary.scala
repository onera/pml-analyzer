...
trait TransactionLibrary {
  ...
  /**
    ...
    */
  final lazy val transactionByUserName: Map[UserTransactionId, PhysicalTransactionId] =
    UsedTransaction.all.flatMap(u => for{t <- u.toPhysical} yield u.userName -> t).toMap
  ...
  final lazy val transactionUserName: Map[PhysicalTransactionId,Set[UserTransactionId]] =
    transactionByUserName
      .keySet
      .groupMap(k => transactionByUserName(k))(k => k).withDefaultValue(Set.empty)

  /**
    * Map from the user defined scenario to the physical transaction id
    * WARNING: this lazy variable can be called ONLY AFTER TRANSACTION/SCENARIO DEFINITION
    * @group user_scenario_relation
    */
  final lazy val scenarioByUserName: Map[UserScenarioId, Set[PhysicalTransactionId]] = {
    (transactionByUserName.keySet.map(k => UserScenarioId(k.id) -> Set(transactionByUserName(k))) ++
  ...
  final lazy val scenarioUserName: Map[Set[PhysicalTransactionId], Set[UserScenarioId]] = {
    val result = scenarioByUserName.keySet.groupMap(k => scenarioByUserName(k))(k => k).withDefaultValue(Set.empty)
  ...
  /**
    ...
    * @group utilFun
    * @param tMap the transaction library to check
    * @param sMap the scenario library to check
    */
  final def checkLibrary(tMap: Map[PhysicalTransactionId,Set[UserTransactionId]],
                   sMap: Map[Set[PhysicalTransactionId], Set[UserScenarioId]]): Unit = {
    for (k<- transactionsByName.keySet) {
      if( (!tMap.contains(k) || tMap(k).isEmpty) && !sMap.keySet.flatten.contains(k))
        println(Message.transactionNoInLibraryWarning(k))
      for {s <- tMap.get(k) if s.size >= 2} yield
        println(Message.transactionHasSeveralNameWarning(k, s))
    }
    this match {
      case i: InterferenceSpecification =>
        for ((s, st) <- i.purifiedScenarios if !sMap.contains(st) || sMap(st).isEmpty) {
  ...
  /**
    ...
    * @param name the name of the transaction or scenario
    */
  ...
  /**
    ...
    * @param userName the name of the node
    * @param iniTgt   a by-name value providing the origin-destination service couples of the transaction (not
    *                 evaluated during the object initialisation)
    * @param sw the application that can use this transaction
    */
  ...
  object Transaction extends PMLNodeBuilder[Transaction] {
    ...
    /**
      ...
      *
      * @param iniTgt the application/target service used
      * @param name   the implicit name of the transaction (deduced from val used during instantiation)
      * @param t      the type tag used to distinguish application loads and stores
      * @tparam A the type of requests
      * @return the transaction (not used for now)
      */
    def apply[A: AsTransaction](iniTgt: => A)(implicit name: Name): Transaction = {
      val result = TransactionParam(iniTgt)
      apply(UserTransactionId(Symbol(name.value)), result._1, result._2)
    }

    /**
      * Main constructor of a transaction, note that transaction are memoized, so if the same name is used in the same platform
      * the constructor will send back the previous definition of the transaction
      *
      * @param name   the name of the transaction
      * @param iniTgt the set of initial/target services defining the transaction
      * @param sw     the applications that may invoke this transaction
      * @param owner  the owner of the transaction (the platform)
      * @return the transaction (not used for now)
      */
    def apply(name: UserTransactionId, iniTgt: () => Set[(Service, Service)], sw: () => Set[Application])(implicit owner: Owner): Transaction = {
      _memo.getOrElseUpdate((owner.s, name.id), new Transaction(name, iniTgt, sw))
    }

    /**
      * A transaction can be from an application targeting a load or a store service
      *
      * @param name   explicit name of the transaction
      * @param iniTgt the application/target service used
      * @param t      the type tag used to distinguish application loads and stores
      * @tparam A the type of requests
      * @return the transaction (not used for now)
    ...
    /**
      ...
      *
      * @param from the initial transaction
      * @param name the implicit name of the transaction (deduced from val used during instantiation)
      * @return the transaction (not used for now)
      */
  ...
  /**
    ...
    * @group scenario_class
    * @param userName the name of the node
    * @param iniTgt   a by-name value providing the origin-destination service couples of the scenario (not
    *                 evaluated during the object initialisation)
    * @param sw the application that can use this scenario
    */
  final class Scenario private(
                  val userName: UserScenarioId,
                  val iniTgt: () => Set[(Service, Service)],
                  val sw: () => Set[Application]) extends ScenarioLike(userName.id) {

    /**
      * Consider the transaction for the analysis
      *
      * @return the used scenario class
      */
  ...
  object Scenario extends PMLNodeBuilder[Scenario] {
    ...
    /**
      ...
      * @param iniTgtL the set of initiator/target of left member
      * @param iniTgtR the set of initiator/target of right member
      * @param name the implicitly derived name
      * @param tA the type tag of left member to solve erasure issue
      * @param tB the type tag of right member to solve erasure issue
      * @tparam A the type of left request
      * @tparam B the type of right request
    ...
    /**
      ...
      * @param tr the original scenario like
      * @param name the implicitly derived name
      * @param t the type tag to solve erasure issue
    ...
    /**
      ...
      * @param name the implicit name of the scenario (same as the variable used to refer to it)
      * @param t    the type tag to distinguish the type of target service
      * @tparam A the type of targeted service
      * @return a scenario
      */
    ...
    /**
      * Main constructor of a scenario, note that scenarios are memoized, so if the same name is used in the same platform
      * the constructor will send back the previous definition of the scenario
      *
      * @param name   the name of the scenario
      * @param iniTgt the set of initial/target services defining the scenario
      * @param sw     the applications that may invoke this scenario
      * @param owner  the owner of the transaction (the platform)
      * @return the transaction (not used for now)
      */
  ...
  /**
    ...
    * @group scenario_class
    * @param userName the name of the node
    * @param iniTgt   the origin-destination services couples
    * @param sw the application that can use this scenario
    */
  final class UsedScenario private(
    ...
    /**
      ...
      *
      * @return the set of physical transaction if possible
      */
    def toPhysical: Set[PhysicalTransactionId] = {
      iniTgt.flatMap(it =>
        transactionsByName.filter(p => p._2.size >= 2 && it._1 == p._2.head && it._2 == p._2.last).toList match {
          case Nil =>
  ...
  object UsedScenario extends PMLNodeBuilder[UsedScenario] {
    ...
    /**
      ...
      * @param iniTgt the initiator/target couples
      * @param sw the application that can use it
      * @param owner the implicitly derived owner of the scenario
      * @return the corresponding scenario (used in the interference analysis)
      */
    def apply(name: UserScenarioId, iniTgt: Set[(Service, Service)], sw: Set[Application])(implicit owner: Owner): UsedScenario = {
      _memo.getOrElseUpdate((owner.s, name.id), new UsedScenario(name, iniTgt, sw))
    }
  }
  ...
  final class UsedTransaction private(
    ...
    /**
      ...
      *
      * @return the physical transaction if possible
      */
    def toPhysical: Option[PhysicalTransactionId] = transactionsByName
      .filter(p => p._2.size >= 2 && iniTgt.exists(it => it._1 == p._2.head && it._2 == p._2.last)).toList match {
      case Nil =>
  ...
  object UsedTransaction extends PMLNodeBuilder[UsedTransaction] {
    ...
    /**
      ...
      * is used in the same platform the constructor will send back the previous definition of the used transaction
      *
      * @param name   the name of the used transaction
      * @param iniTgt the set of inital/target services defining the transaction
      * @param sw     the application that may invoke this transaction
      * @param owner  the platform owning the transaction
      * @return the used transaction
      */
    def apply(name: UserTransactionId, iniTgt: Iterable[(Service, Service)], sw: Set[Application])(implicit owner: Owner): UsedTransaction = {
      _memo.getOrElseUpdate((owner.s, name.id), new UsedTransaction(name, iniTgt, sw))
    }
  ...
  /**
    ...
    * @param x id of the user transaction
    */
  final implicit class UserTransactionOps(x: UserTransactionId) extends TransactionLikeOps {
    def paths: Set[PhysicalTransaction] =
      (for {
        id <- transactionByUserName.get(x)
      } yield
        Set(transactionsByName(id))) getOrElse Set.empty
  }

  /**
    * Scenario extension method
    * @group scenario_operation
    * @param x id of the user scenario
    */
  final implicit class ScenarioOps(x: UserScenarioId) extends TransactionLikeOps {
    def paths: Set[PhysicalTransaction] =
