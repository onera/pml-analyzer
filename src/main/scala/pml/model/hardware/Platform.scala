...
/**
  ...
  * @param _sourceFile the implicit descriptor of the source file where the platform is defined
  * @group hierarchical_class
  */
abstract class Platform(val name: Symbol)(implicit _sourceFile: File) extends PMLNode with Relation.Instances {
  ...
  /**
    ...
    * @group transaction
    */
  final lazy val transactionsByName: Map[PhysicalTransactionId, PhysicalTransaction] =
  ...
  /**
    ...
    * @group transaction
    */
  final lazy val transactions: Set[PhysicalTransactionId] = transactionsByName.keySet
  ...
  /**
    ...
    * @group transaction
    */
  final lazy val transactionsBySW: Map[Application, Set[PhysicalTransactionId]] =
    Application.all.groupMapReduce(a => a)(a => {
        ...
        transactionsByName.collect({
          case (id, path) if targetServices.contains(path.last) && initServices.contains(path.head) => id
        }).toSet
    })(_ ++ _)


  private val _transactionId = collection.mutable.HashMap.empty[PhysicalTransaction, PhysicalTransactionId]
  private val _scenarioId = collection.mutable.HashMap.empty[PhysicalScenario, PhysicalScenarioId]

  /**
    * Build the transaction id as "head_last_i" where i is the number of path with the same
    * origin and destination as the one on build (possible when multiple paths in the architecture)
    *
    * @param t the sequence of services
    * @return the unique transaction id
    */
  protected final def transactionId(t: PhysicalTransaction): PhysicalTransactionId = _transactionId.getOrElseUpdate(t, {
    val sameHT = _transactionId.keys.count(tp => t.head == tp.head && t.last == tp.last)
  ...
  /**
    ...
    * @group transaction
    */
  final lazy val transactionsName: Map[PhysicalTransaction, PhysicalTransactionId] =
    transactionsByName.groupMapReduce(_._2)(_._1)((l, _) => l)

  /**
    * Build the scenario id as "t_1|...|t_n"
    *
    * @param s the set of physical transactions forming the scenario
    * @return the unique id of the scenario
    */
  final def scenarioId(s: PhysicalScenario): PhysicalScenarioId = _scenarioId.getOrElseUpdate(s, {
    PhysicalScenarioId(Symbol(s.map(t => t.id.name).toArray.sorted.mkString("|")))
  })
  ...
  protected trait TransactionLikeOps {
    ...
    /**
      ...
      * @return true if the target is contained
      */
    ...
    /**
      ...
      */
    def target: Set[Target] = paths.filter(_.size >= 2).flatMap(t => t.last.targetOwner)

    /**
      * Provide the initiators fo a transaction
      * @return the set of initiators
      */
    def initiator: Set[Initiator] = paths.filter(_.nonEmpty).flatMap(t => t.head.initiatorOwner)

    /**
      * Check is the initiator is in the possible initiators of the transaction
      * @param x initiator to find
      * @return true if the initiator is contained
      */
    def initiatorIs(x: Initiator): Boolean = initiator.contains(x)

    /**
      * Check if the transaction is a load transaction
      * @return true if target services are loads
      */
    def isLoad: Boolean = paths.forall(path => path.nonEmpty && path.head.isInstanceOf[Load])

    /**
      * Check if the transaction is a store transaction
      * @return true if target services are stores
      */
    def isStore: Boolean = paths.forall(path => path.nonEmpty && path.head.isInstanceOf[Store])
  }

  /**
    * Extension methods for physical transaction identifiers
    * @param x the name of the physical transaction
    */
  final implicit class PhysicalTransactionOps(x: PhysicalTransactionId) extends TransactionLikeOps {
