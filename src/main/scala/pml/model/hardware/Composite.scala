...
/**
  ...
  * @param _owner the id of the owner of the composite (the platform or another composite)
...
abstract class Composite(n: Symbol, _owner:Owner) extends Hardware {
  ...
  /**
    ...
    * @note the initial value of the owner is stored in [[owner]]
    * @group identifier
    */
  ...
  /**
    ...
    * @group component_access
    * @return set of all sub-components
    */
...
object Composite extends PMLNodeBuilder[Composite] {
  ...
  /**
    ...
    */
  private def formatName(name:Symbol, owner: Owner): Symbol = Symbol(owner.s.name+"_"+name.name)
  ...
  /**
    ...
    */
  private def add(c: Composite, owner: Owner): Unit = _memo.addOne((owner.s, c.name), c)
}
